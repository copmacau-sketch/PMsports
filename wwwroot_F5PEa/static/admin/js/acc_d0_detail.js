function acc_d0_detail(_win, _dom, paramObj){
    var _self = this;
    var level = "d0";
    var win = _win;
    var dom = _dom;
    ...
    acc_title["summary"] = new Array("summary_co","summary_su","summary_ag","summary_mem");
    ...
    _self.init=function(){
        ...
        obj_ids += "box_possess,box_possess,input_possess_d0,sel_possess_d0,possess_tip,possess_effect_time,msg_possess,";
        obj_ids += "box_acc_summary,amount_co,amount_su,amount_ag,amount_mem,last_login,last_chg_date,";
        obj_ids += "box_perf,perf_yesterday,perf_week,perf_period,perf_winloss,perf_profit,perf_turnover,";
        obj_ids += "box_down_acc_summary,summary_co,summary_su,summary_ag,summary_mem,amount_active,amount_veiwonly,amount_inactive,amount_suspended,";
    ...
    _self.loadUserData = function(){
        var par = "p=get_" + level + "_list" + _self.targetParam("get_list_d0");
        var getHttp = new HttpRequest();
    ...
    _self.upd_data = function(e){
        ...
        var new_winloss = _mc["sel_possess_d0"].value;
        var new_enddate = 0;
        ...
        if( (new_enddate != acc_par.enddate) || (new_alias != acc_par.alias) || (new_credits*1 != acc_par.maxcredit*1) || (new_status != acc_par.status) || (new_psw_recv != acc_par.psw_enable) || (new_winloss != acc_par.dfwinloss) ){
            ...
            var dfwinloss = _mc["sel_possess_d0"].value;
            acc_par.dfwinloss = dfwinloss;
            var ret = _self.submitCheck();
            if(!ret)	return;
            //util.removeEvent(_mc["btn_save"], "click");

            var par = "p=body_d0_edit" + _self.targetParam("body_d0_edit");
            var getHttp = new HttpRequest();
    ...
    _self.showUserCredit = function(data){
        ...
        //可設定成數範圍
        d0_max = config_set.get("D0_WINLOSS_MAX");
        _mc["input_possess_d0"].innerHTML = _self.formatePossess(arr_data["winloss"]);
        if(arr_data["dfwinloss_sw"]=="Y"){
            _self.initOption(_mc["sel_possess_d0"], arr_data["dfwinloss"], d0_max, 1);
        ...
        acc_par.dfwinloss = _mc["sel_possess_d0"].value;
    }
    ...
    _self.targetParam = function(targetName){
        ...
        }else if(targetName == "get_list_d0"){
            par+="&aid="+param.edit_id;
