function bet_edit_index(_win, _dom,_toppar){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    var classname = "bet_edit_index";
    var util;
    var LS;
    var config_set;
    var Timer;
    ...
    _self.initFilterParam = function(top_filter,top_filterFlag){
        ...
        if(top_filter && top_filterFlag){
            for(var key in top_filter){
                ...
                if(key=="gtype" || key=="result" || key=="ctype"){
    ...
    _self.reloadData=function(){
        ...
        par+="&gtype="+_set["gtype"];
        par+="&result="+_set["result"];
        /*par+="&downline="+filterUse["downline"];
        par+="&event="+filterUse["event"];
        par+="&league="+filterUse["league"];*/
    ...
    _self.loadFinish=function(json){
        ...
        for(var prop in allwagers){
            ...
            var cname = allwagers[prop].CNAME;
            var ctype = allwagers[prop].CTYPE;

            if(!_self.chkCanParse(gtype,stake,downline,market,league,_event,dates,cname,ctype))	continue;
            ...
            for(var j=0;j<ret_xmp.length;j++){
                ...
                xmp_tmp = xmp_tmp.replace(new RegExp("\\\*LOG\\\*", "gi"), allwagers[prop].EDITLOG);
                ...
                xmp_tmp = xmp_tmp.replace(new RegExp("\\\*AD\\\*", "gi"), allwagers[prop].ADRESULT);*/
...
//console.log(outStr_small);
    ...
    _self.loadFilterFinish = function(json){
        ...
        var team = hash["team"];
        var cname = hash["cname"];
        var outStr="";
        data_filter = hash;
        _self.setSelectEvent();
    }

    _self.chkCanParse=function(gtype,stake,downline,market,league,_event,dates,cname,ctype){
        ...
        if(filterUse["ctype"] != 0){
            if(filterUse["ctype"]!=ctype){
                return false;
            }
        }

        if(filterUse["cname"] != "ALL"){
            var checkAry = filterUse["cname"].toString().split(",");
            if(checkAry.indexOf(cname)*1 == -1){
                return false;
            }
        }
    ...
    _self.createWmcTimer=function(){
        ...
        timerHash["bet"] = new Timer(config_set.get("BETLIST_RELOAD"));
        timerHash["bet"].setParentclass(_self);
        timerHash["bet"].dont_clear = false; //設定為不清除timer
        timerHash["bet"].init();
        timerHash["bet"].addEventListener("TimerEvent.TIMER", _self.betTimerRun);
        timerHash["bet"].addEventListener("TimerEvent.TIMER_COMPLETE", _self.betTimerFinish);
        timerHash["bet"].startTimer();
    ...
    _self.stopDangerTimer = function () {
        ...
        timerHash["bet"].stopTimer();
    }

    _self.clearDangerTimer=function(){
        util.echo("bet clearTimer");
        if(timerHash["bet"]!=null){
            timerHash["bet"].clearObj();
            timerHash["bet"]=null;
        }
        return true;
    }
    ...
    /*	舊管理端使用的
        _self.createTimer=function(){
            ...
            console.log("SCR===>"+sec);
            _myTop["timerHash"]["bet"+_set["gtype"]] = new _mc["Timer"].JS_Obj(sec);
            _myTop["timerHash"]["bet"+_set["gtype"]].setParentclass(_self);
            _myTop["timerHash"]["bet"+_set["gtype"]].addEventListener("TimerEvent.TIMER", _self.timerRun);
            _myTop["timerHash"]["bet"+_set["gtype"]].addEventListener("TimerEvent.TIMER_COMPLETE", _self.timerFinish);
            _myTop["timerHash"]["bet"+_set["gtype"]].start();

            _self.debugPrint("createTimer===>"+_myTop["timerHash"]["bet"+_set["gtype"]].isClear);
        }
        ...
        _self.clearTimer=function(){
            ...
            if(_myTop["timerHash"]["bet"+_set["gtype"]] != null){
                _myTop["timerHash"]["bet"+_set["gtype"]].clearObj();
            }
        }
...
//=====
    ...
    _self.setSelectEvent=function(){
        ...
        if(filterBigObj==null){
            ...
            var tmpObj = new Object();
            var tmpAry = ["league","event","downline","dates","cname"];
            for(var i=0;i<tmpAry.length;i++){
                ...
                if(data_filter[rtype]){
                    ...
                    if(rtype == "downline"){
                        ...
                        tmpObj[rtype] = {"_data":data_filter[rtype],"_default":filterInitParam["cname"]["_default"],"_breakpoint":tmp};
                    }else{
    ...
    _self.takeParam = function(obj){
        ...
        if(_set["ctype"] != filterUse["ctype"])
        {
            _set["ctype"] = filterUse["ctype"];
        }
