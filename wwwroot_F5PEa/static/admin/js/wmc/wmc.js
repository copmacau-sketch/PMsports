function wmc(_win, _dom, param){
	var _self=this;
	var _top=null;
    var win = _win;
    var dom = _dom;
	var classname = "wmc.js";
	var tmpScreen;
	var paramObj;
	var parentClass;
    var util;
	...
	var storage;
	var data_filter;
	var levelClassAry = ["AGCLASS","MACLASS","SMACLASS","D0CLASS","ADCLASS"];
	var levelNum = 0;
	...
	_set["tdnoneSMA"] = "";
	_set["tdnoneCO"] = "";
	_set["width_sty"] = "bet_wmcDetail22";
	_set["gtype"] = "ALL";
	_set["sel_maxid"] = "0";
	_set["gold"] = "";//stake
	_set["down_id"] = "";
	...
	_self.init=function(){
		...
		util.setInfEvent(dom.getElementById("wmc_burger_div"), { "_focus": dom.getElementById("wmc_burger_sel"), "_setView": dom.getElementById("wmc_burger_div"), "_viewClass": "active" });

		// _self.bigOrSmall();

		var storage_filter_wmc =  storage.get(top.login_layer+"_"+top.user_id+"_filter_wmc");
		var storage_remenberflg_wmc=  storage.get(top.login_layer+"_"+top.user_id+"_remenberflg_wmc");
		var filter_wmc_Obj;
		var remenberflg_wmc_Obj;
		// console.log(storage_filter_wmc,storage_remenberflg_wmc);
		if(storage_filter_wmc!="null") filter_wmc_Obj = JSON.parse(storage_filter_wmc);
		if(storage_remenberflg_wmc!="null" && typeof(storage_remenberflg_wmc)!='undefined')  remenberflg_wmc_Obj = JSON.parse(storage_remenberflg_wmc);

		_self.initFilterParam(filter_wmc_Obj,remenberflg_wmc_Obj);
		...
		// 	_set["width_sty"] = "bet_wmcDetail32";
		...
		// 	_set["width_sty"] = "bet_wmcDetail42";
		// 	var tdnoneCO_sty = util.getObjectClass(_mc["tdnoneCO"]);
		// 	var tdnoneSMA_sty = util.getObjectClass(_mc["tdnoneSMA"]);
	...
	_self.loadFinish=function(json){
		...
		if(xmlObj["wagers"].length > 0){
			...
			for(var i=0; i<xmlObj["wagers"].length; i++){
				...
				xmlObj["ad"] = xmlnode.Node(xmlObj["wagers"][i],"ad");
				var ad = util.showTxt(xmlnode.getNodeVal(xmlObj["ad"]));
...
/*	舊管理端使用的
	_self.createTimer=function(){
		...
		console.log("SCR===>"+sec);
		_myTop["timerObj"]["wmc"+_set["gtype"]] = new _mc["Timer"].JS_Obj(sec);
		_myTop["timerObj"]["wmc"+_set["gtype"]].setParentclass(_self);
		_myTop["timerObj"]["wmc"+_set["gtype"]].addEventListener("TimerEvent.TIMER", _self.timerRun);
		_myTop["timerObj"]["wmc"+_set["gtype"]].addEventListener("TimerEvent.TIMER_COMPLETE", _self.timerFinish);
		_myTop["timerObj"]["wmc"+_set["gtype"]].start();

		_self.debugPrint("createTimer===>"+_myTop["timerObj"]["wmc"+_set["gtype"]].isClear);
	}
	...
	_self.clearTimer=function(){
		...
		if(_myTop["timerObj"]["wmc"+_set["gtype"]] != null){
			_myTop["timerObj"]["wmc"+_set["gtype"]].clearObj();
		}
	}
			...
			for(var rtype in filterInitParam){
				...
				if(rtype=="gtype" || rtype=="market"){
					...
					for(var i=0;i<_list.length;i++){
						filterInitParam[rtype]["_listSub"][i] = LS.get("str_wmc_"+status+_list[i]);
