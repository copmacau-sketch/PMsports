function acc_co_detail(_win, _dom, paramObj){
    var _self = this;
    var level = "co";
    var win = _win;
    var dom = _dom;
    ...
    acc_title["summary"] = new Array("summary_su","summary_ag","summary_mem");
    ...
    _self.init=function(){
        ...
        obj_ids += "box_acc_summary,amount_su,amount_ag,amount_mem,last_login,last_chg_date,";
        obj_ids += "box_perf,perf_yesterday,perf_week,perf_period,perf_winloss,perf_profit,perf_turnover,";
        obj_ids += "box_down_acc_summary,summary_su,summary_ag,summary_mem,amount_active,amount_veiwonly,amount_inactive,amount_suspended,";
    ...
    _self.showUserData = function(data){
        ...
        if(!arr_data["account"].hasOwnProperty("special") || arr_data["account"]["special"] == "N"){
            if(_mc.hasOwnProperty("acc_power")){
                _mc["acc_power"].remove();
            }
        }else{
            _mc["acc_power"].style.display = "";
        }
