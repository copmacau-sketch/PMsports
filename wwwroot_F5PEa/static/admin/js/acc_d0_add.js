function acc_d0_add(_win, _dom, paramObj){
    var _self = this;
    var win = _win;
    var dom = _dom;
    ...
    _self.init=function(){
        ...
        _self.linkage_d1 = function(){
            ...
            document.getElementById("D1_MEM_LB").addEventListener('click',function(){
                ...
                if(aMem3 !== aMem5){
                    for(var i=0;i<3;i++){
                        check_mem1[i].checked = true;
                    }
                }else{
                    for(var j=0;j<check_mem1.length;j++){
                        check_mem1[j].checked = false;
                    }
                }
            })
        }
    ...
    _self.initWinloss = function () {
        ...
        var su_r = config_set.get("CO_WINLOSS_RANGE");
        var su_max = config_set.get("CO_WINLOSS_MAX");
        var percent = "%";
        for (i = 0; i < su_max + su_r; i += su_r) {
            var _value = 100 - i;
            var _txt = i;
    ...
    _self.getSaveParam = function () {
        ...
        par += "&winloss=" + config_set.get("ADD_D0_CO_WINLOSS");
        par += "&winloss_c=" + _mc["input_possess"].value;
