function match_header(_win, _dom, _toppar){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    var classname = "match_header";
    var util;
    var LS;
    var LS_code;
    ...
    var toppar = _toppar;
    var showtypeAry = new Array("MATCH","SCORE");
    //var gtypeAry = new Array("FT","BK","TN","VB","BM","TT","BS","SK","OP");

    _self.init=function(){
        if(toppar==null)toppar = new Object();
        // console.log("[match_header][init]",toppar);
        util.echo("match_header complete");
        ...
        if(top.tbet_showtype==null)top.tbet_showtype = "MATCH"; //預設赛程
        if(top.tbet_gtype==null)top.tbet_gtype = "FT"; //預設赛程
        if(top.score==null)top.score = "N";
        if(top.bet==null)top.bet = "Y";
        if(toppar["sub_page"] == null) toppar["sub_page"] = "match_index";
        ...
        if(toppar["bet"]==null)toppar["bet"] = top.bet;
        if(toppar["score"]==null)toppar["score"] = top.score;
    ...
    _self.subPage = function(par){
        // if(par["sub_page"] == "result_index" || par["sub_page"] == "match_index" || par["sub_page"] == "parlay_index"){
            // _self.getTbetIndex(par,true);
        // }
        if(par["sub_page"] == "match_wagers_index"){
    ...
    _self.chgShowType = function (e, param) {
        ...
        if(toppar["tbet_showtype"] == "MATCH"){
            toppar["sub_page"] = "match_index";
        }else if(toppar["tbet_showtype"] == "SCORE"){
    ...
    _self.getTbetIndex = function (par,isInit) {
        ...
        if(isInit){
            ...
            if(par["tbet_showtype"] == "MATCH"){
                param["page"] = "match_index";
            }else if(par["tbet_showtype"] == "SCORE"){
                param["page"] = "score_index";
            }else{
                param["page"] = "match_index";
            }
            //param["page"] = "match_index";
            param["target"] = "match_body";
            param["useDefineParent"] = "Y"; //match_index載進來才會再執行init();
            param["postHash"] = par;
