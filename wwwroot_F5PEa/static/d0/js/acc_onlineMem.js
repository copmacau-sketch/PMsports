function acc_onlineMem(_win, _dom,param){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    ...
    _self.connetToServer=function(){
        ...
        urlParams += "&view_layer="+paramHash["edit_layer"];
        urlParams += "&view_id="+paramHash["edit_id"];
        urlParams += "&chk_mem=N";
    ...
    _self.parseData = function (hash) {
        ...
        var memAry = new Array("memcount","username","loginname","alias","lastdate","ip","url","id");
        for(var i=0; i<hash.length; i++){
            tpl.addBlock("onlineM");
            for(var a=0; a<memAry.length; a++){
                var keys = memAry[a];
                if(keys == "ip" && hash[i]["ip"]!=null){
                    var str = '<a href="http://www.ip38.com/ip.php?ip='+hash[i][keys]+'" target="_blank"  class="word_blue" title="点击查看IP归属地">'+hash[i][keys]+'</a>';
                    if(hash[i].hasOwnProperty("sp")){
                        var filename = "app/record/index.php";
                        filename += "?langx="+top.langx;
                        filename += "&uid="+top.uid;
                        filename += "&login_layer="+top.login_layer;
                        filename += "&id="+hash[i]["id"];
                        filename += "&layer="+layer;
                        str += "/";
                        str += '<a href="'+filename+'" target="_blank"  class="word_blue" title="点击查看活动轨迹">';
    ...
    _self.initEvent = function (ArrObj, div_show) {
        for (var i = 0; i < ArrObj.length; i++) {
            ...
            var obj_ids = ",btn_out,btn_bet,btn_proNews,btn_proChat,btn_sp,";
            var _ary = util.getObjAry(div, obj_ids);
            util.addEvent(_ary["btn_out"], "click", _self.outEvent, obj);
            util.addEvent(_ary["btn_bet"], "click", _self.betEvent, obj);
            util.addEvent(_ary["btn_proNews"], "click", _self.proNewsEvent, obj);
            util.addEvent(_ary["btn_proChat"], "click", _self.proChatEvent, obj);
            util.addEvent(_ary["btn_small_out"], "click", _self.outEvent, obj);
            util.addEvent(_ary["btn_bet_small"], "click", _self.betEvent, obj);
            util.addEvent(_ary["btn_proNews_small"], "click", _self.proNewsEvent, obj);
            util.addEvent(_ary["btn_proChat_small"], "click", _self.proChatEvent, obj);
