function game_list(_win, _dom, _post) {
    ...
    var OBT_needsR = new Array("CN","RN","WI","ET","PK","PD");
    ...
    var config_ior = null;
    var needsTransWtype = new Array("RG","RPX","RSH","RSC","RNC","RNB","RF","RGA");
    var sort_type = "L";
    var chgSort = false;
    ...
    _self.chgTabCss = function(_rtype) {
        for (var i = 0; i < TAB_ary.length; i++) {
            var tmpObj = dom.getElementById("tab_" + TAB_ary[i]);
            if (tmpObj != null)
        ...
        if (_rtype != "r" && _rtype != "rb")
            if (top.choice_showtype == "live") {
                ...
                tmpRtype = _rtype;
        var obj = dom.getElementById("tab_" + tmpRtype);
        if (obj)
    ...
    _self.gameTimerRun = function(count) {
        if (top.specialClick == "special") {
            ...
            if (top.choice_gtype == "ft" && top.choice_showtype != "mygame")
                parentClass.dispatchEvent("getPgCnt");
            _self.getData()
        }
    }
    ...
    _self.getModel = function(_par) {
        ...
        param["post"] = "gtype=" + top.choice_gtype + "&showtype=" + showtype + "&rtype=" + rtype + "&isSpecial=" + isSpecial + "&isFantasy=" + isFantasy + "&ts=" + _par.chgModelTs;
    ...
    _self.LoadGameComplete = function(xml) {
        ...
        var code = _xmlnode.Node(_xmlnode.Root[0], "code").innerHTML;
        ...
        if (postHash["back"] != "Y" && nowClickTabTs && nowClickTabTs != "")
            if (nowClickTabTs != top["specialGame"]["clickTabTs"]) {
                console.log("\u7279\u6b8aTS\u4e0d\u540c\uff0c\u963b\u64cb!!!! [ts] = ", nowClickTabTs, "top\u503cts = ", top["specialGame"]["clickTabTs"]);
                return
            }
    ...
    ;
    _self.parseData = function(xmlnode, isClick) {
        var _header = "";
        var couponKey = "";
        ...
        var couponLid = xmlnode.Node(xmlnode.Root[0], "couponLid").innerHTML;
        ...
        try {
            if (top.specialClick == "special") {
                ...
                if (top.specialGame.cup_page == "game" || top.specialGame.mode == "NORMAL" && !top.specialGame.isFantasy)
                    sportFrame.showSelectSort({
                        "isShow": true
                    });
                else
        ...
        if (xmdObj["ec"].length > 0 && showData) {
            if (!isAllZero)
    ...
    _self.getScDataObj = function(tmp_game, mainGame, showtype) {
        ...
        if (nowShowtype.match(/mygame|today|hot/) || isSpecialGame == "Y")
            obj.showtype = util_game.transMyGameShowtype(tmp_game["myGame"]);
        else
        ...
        if (tmp_game["datetime"] != null && tmp_game["datetime"] != "") {
            ...
            if (Math.abs(diff) > 0) {
                ...
                if (newDay != str_D * 1)
                    obj.newDatetime = newDay + " / " + newMonth + "<b></b>" + newHour + ":" + newMin;
                else
                    obj.newDatetime = isToday ? newHour + ":" + newMin : newDay + " / " + newMonth + "<b></b>" + newHour + ":" + newMin
            } else
    ...
    _self.workerPost = function(WORKER, parObj) {
        ...
        for (var _key in dataHash) {
            ...
            if (top.choice_rtype.match(/pd/)) {
                ...
                if (top.nowPDMode == "choice") {
                    ...
                    else if (pdSortHash[_key + halfStr] && pdSortHash[_key + halfStr]["choice"] != "" && tmpPDRtype.match(pdSortHash[_key + halfStr]["choice"]))
        ...
        if (top.rightECID != "")
            if (dataHash["ec" + top.rightECID] != null && xmlHash["ec" + top.rightECID] != null && filterRtypeAry.indexOf(top.choice_rtype) == -1) {
    ...
    _self.getRatioLayer = function(_name, is_rb, isfantasy, pdStrongModel, isHalf) {
        ...
        var todayFantasy = top.specialGame.isFantasy && top.choice_showtype == "today" && top.specialClick == "";
        if (top.specialClick != "" && top.specialGame.isFantasy && isfantasy == "Y")
            tmpName = _name;
        if (top.choice_rtype.match(/pd/)) {
            var tmpStr = "DRAW";
            if (top.nowPDMode == "choice")
            ...
            if (isHalf == "Y")
                if (isMix == "Y" && _name.match(/HT|FT/) && is_rb == "N" && (!top.specialGame.isFantasy || todayFantasy))
                    tmpName = "HT_R_" + tmpStr;
                else
                    tmpName = "HT_" + tmpStr;
            else if (isMix == "Y" && _name.match(/HT|FT/) && is_rb == "N" && (!top.specialGame.isFantasy || todayFantasy))
    ...
    _self.initInfoBtn = function(xmlnode, ecObj) {
        ...
        for (var j = 0; j < ecObj.length; j++) {
            ...
            var _hgid = xmlnode.Node(tmp_game, "HGID").innerHTML;
            var _gidm = xmlnode.Node(tmp_game, "GIDM").innerHTML;
            ...
            var more_team_c = xmlnode.Node(tmp_game, "team_c").innerHTML;
            ...
            var SFSGAME = xmlnode.Node(tmp_game, "SFSGAME").innerHTML;
            var MIDFIELD = xmlnode.Node(tmp_game, "MIDFIELD").innerHTML;
            var HNIKE = xmlnode.Node(tmp_game, "HNIKE").innerHTML;
            ...
            var objids = top.choice_gtype == "ft" ? ",div_icon_info,info_team,info_more,info_icon_btn,icon_N," : ",div_icon_info,more,";
            var ary = util.getObjAry(infoObj, objids);
            var needExtraInfo = ptype_str[ptype_map] == "PK" || ptype_str[ptype_map] == "ET" || top.choice_gtype == "ft" && MIDFIELD == "Y";
            if (ary["div_icon_info"]) {
                var dataOBj = new Object;
                if (ptype_str[ptype_map] == "PK" || ptype_str[ptype_map] == "ET") {
                    var FT_score_h = xmlnode.Node(tmp_game, "FT_SCROE_H").innerHTML;
    ...
    _self.initIorBtn = function(xmlnode, ecObj) {
        ...
        var gtype_setAry = new Array("bk","bs","bm","tt","vb","tn","sk");
        var myGameShowtype = "";
        var isSpecialGame = top.specialClick != "" && !top.specialGame.isFantasy && top.choice_rtype != "fs" ? "Y" : "N";
        for (var j = 0; j < ecObj.length; j++) {
            var tmp_ec = ecObj[j];
            ...
            for (var x = 0; x < xmdObj["game"].length; x++) {
                ...
                var gidm = xmlnode.Node(tmp_game, "GIDM").innerHTML;
                                ...
                                if (tmp_gid) {
                                    ...
                                    if (obj) {
                                        ...
                                        if (util.in_array(_wtype, needsTransWtype)) {
                                            var tmpWtype = "";
                                            if (top.choice_gtype == "tn" && _wtype.indexOf("RGA") != -1) {
                                                var RGA_TN = xmlnode.Node(tmp_game, "RGA", false);
                                                if (RGA_TN.length > 0) {
                                                    for (var w = 0; w < RGA_TN.length; w++)
                                                         var tmpWtype = RGA_TN[w].getAttribute("wtype");
                                                    if (tmpWtype == "")
                                                        continue
                                                }
                                            } else if (top.choice_gtype == "tn" && _wtype.indexOf("RF") != -1) {
                                                var RF_TN = xmlnode.Node(tmp_game, "RF", false);
                                                if (RF_TN.length > 0) {
                                                    for (var w = 0; w < RF_TN.length; w++)
                                                         var tmpWtype = RF_TN[w].getAttribute("wtype");
                                                    if (tmpWtype == "")
                                                        continue
                                                }
                                            } else
                                                tmpWtype = util_game.showTxt(xmlnode.Node(tmp_game, "WTYPE_" + _wtype.toUpperCase()).innerHTML);
                                        ...
                                        if (_gid != null && ECID != null && nowRtype != null) {
                                            ...
                                            if ((top.choice_gtype == "tn" || top.choice_gtype == "sk") && nowRtype.match("RF"))
                                                iorRtype = "RF" + nowRtype.substr(-1);
                                            else if (top.choice_gtype == "tn" && nowRtype.match("RGA"))
                                                iorRtype = "RGA" + nowRtype.substr(-1);
                                            var str_ms = "";
                                            if (ms_gid == _gid)
                                                str_ms = "ms_";
    ...
    _self.parseOBTData = function(xmlnode) {
        ...
        if (code == "noData") {
            ...
            var ary = util.getObjAry(menuObj, objids);
            var OBT_noData = ary["OBT_" + (model == "TQ" ? "WI" : nowOBT_Model)];
            if (OBT_noData != null) {
    ...
    _self.initOBTIorBtn = function(xmlnode, ecObj) {
        ...
        for (var j = 0; j < ecObj.length; j++) {
            ...
            for (var x = 0; x < xmdObj["game"].length; x++) {
                ...
                if ((top.choice_showtype == "live" || isRP3 || isMyGameRB || isSpecialGame == "Y" && is_rb == "Y") && util.in_array(model, OBT_needsR))
    ...
    ;
    _self.parseOtherData = function(xmlnode) {
        var div_show = dom.getElementById("div_show");
        var xmdObj = new Object;
        var tmpDiv = "";
        var gidAry = new Array;
        ...
        var tmpTS = xmlnode.Node(xmlnode.Root[0], "ts").innerHTML;
        if (!util_game.checkTS(nowTS, tmpTS, "get_game_list"))
            return;
        xmdObj["ec"] = xmlnode.Node(xmlnode.Root[0], "ec", false);
        var specialTitle = top.specialGame.title != "" ? top.specialGame.title : postHash.specialTitle;
        ...
        for (var j = 0; j < xmdObj["ec"].length; j++)
            try {
                ...
                for (var x = 0; x < xmdObj["game"].length; x++) {
                    ...
                    var tmp_game = xmdObj["game"][x];
                    var tmp_game_copy = xmdObj["game"][x];
                    tmpModel = div_model.cloneNode(true);
                    tmpInfoModel = infoModel.cloneNode(true);
                    if (nowModel == "")
                        nowModel = "R1";
                    tmpLayer = _self.getRatioLayer(nowModel).cloneNode(true);
                    var gid = xmlnode.Node(tmp_game, "GID").innerHTML;
                    gidAry.push(gid);
                    defHash[gid] = tmp_game;
                    if (top.choice_showtype != "parlay") {
                        var ior_reh = xmlnode.Node(tmp_game_copy, "ior_REH").innerHTML;
                        var ior_rec = xmlnode.Node(tmp_game_copy, "ior_REC").innerHTML;
                        var hash_re = util_game.chgOddfIoratio(ior_reh, ior_rec, config_ior);
                        xmlnode.Node(tmp_game_copy, "ior_REH").innerHTML = util.showTxt(hash_re[0]);
                        xmlnode.Node(tmp_game_copy, "ior_REC").innerHTML = util.showTxt(hash_re[1]);
                        var ior_rh = xmlnode.Node(tmp_game_copy, "ior_RH").innerHTML;
                        var ior_rc = xmlnode.Node(tmp_game_copy, "ior_RC").innerHTML;
                        var hash_r = util_game.chgOddfIoratio(ior_rh, ior_rc, config_ior);
                        xmlnode.Node(tmp_game_copy, "ior_RH").innerHTML = util.showTxt(hash_r[0]);
                        xmlnode.Node(tmp_game_copy, "ior_RC").innerHTML = util.showTxt(hash_r[1]);
                        var ior_rouh = xmlnode.Node(tmp_game_copy, "ior_ROUH").innerHTML;
                        var ior_rouc = xmlnode.Node(tmp_game_copy, "ior_ROUC").innerHTML;
                        var hash_rou = util_game.chgOddfIoratio(ior_rouh, ior_rouc, config_ior);
                        xmlnode.Node(tmp_game_copy, "ior_ROUH").innerHTML = util.showTxt(hash_rou[0]);
                        xmlnode.Node(tmp_game_copy, "ior_ROUC").innerHTML = util.showTxt(hash_rou[1]);
                        var ior_ouh = xmlnode.Node(tmp_game_copy, "ior_OUH").innerHTML;
                        var ior_ouc = xmlnode.Node(tmp_game_copy, "ior_OUC").innerHTML;
                        var hash_ou = util_game.chgOddfIoratio(ior_ouh, ior_ouc, config_ior);
                        xmlnode.Node(tmp_game_copy, "ior_OUH").innerHTML = util.showTxt(hash_ou[0]);
                        xmlnode.Node(tmp_game_copy, "ior_OUC").innerHTML = util.showTxt(hash_ou[1])
                    }
                    var InfoAry = GameInfo[top.choice_showtype];
                    for (var i = 0; i < InfoAry.length; i++) {
                        var keys = InfoAry[i];
                        var vals = xmlnode.Node(tmp_game, keys).innerHTML;
                        if (keys == "BS_ICON") {
                            var BASE_1B = xmlnode.Node(tmp_game, "BASE_1B").innerHTML;
                            var BASE_2B = xmlnode.Node(tmp_game, "BASE_2B").innerHTML;
                            var BASE_3B = xmlnode.Node(tmp_game, "BASE_3B").innerHTML;
                            vals = "bs_" + BASE_1B + BASE_2B + BASE_3B
                        } else if (keys == "RB_SHOW") {
                            var rb_display = xmlnode.Node(tmp_game, "RUNNING").innerHTML;
                            vals = rb_display == "Y" ? "" : "none";
                            if (top.choice_showtype == "live")
                                vals = "none"
                        } else if (keys == "DATETIME") {
                            var sys_time = xmlnode.Node(tmp_game, "SYSTIME").innerHTML;
                            vals = _self.transDate(vals, sys_time, false);
                            ...
                            if (point_num == null)
                                point_num = 1;
                            tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(/\*SERVE_H\*/gi, point_tag == "Bottom" ? "on" : "");
                            tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(/\*SERVE_C\*/gi, point_tag == "Top" || point_tag == "Straight" ? "on" : "")
                        } else if (keys == "NOWSERVER") {
                            var isH = top.choice_gtype == "tn" && vals == "1" || top.choice_gtype != "tn" && vals == "0";
                            var isC = top.choice_gtype == "tn" && vals == "0" || top.choice_gtype != "tn" && vals == "1";
                            tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(/\*SERVE_H\*/gi, isH ? "on" : "");
                            tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(/\*SERVE_C\*/gi, isC ? "on" : "")
                        } else if (keys == "RETIMESET") {
                            var typeShow = top.choice_gtype;
                            if (top.choice_showtype == "live")
                                if (typeShow == "bk") {
                                    var se_now = xmlnode.Node(tmp_game, "NOWSESSION").innerHTML;
                                    var lastT = xmlnode.Node(tmp_game, "LASTTIME").innerHTML;
                                    if (se_now == "H1")
                                        se_now = "1H";
                                    if (se_now == "H2")
                                        se_now = "2H";
                                    ...
                                    var best = xmlnode.Node(tmp_game, "BEST").innerHTML;
                                    var mode = best.split(" ");
                                    var se_sum = mode[2];
                                    if (typeShow == "tn") {
                                        sum_h = xmlnode.Node(tmp_game, "SCOREGAMEH").innerHTML * 1;
                                        sum_c = xmlnode.Node(tmp_game, "SCOREGAMEC").innerHTML * 1
                                    } else {
                                        sum_h = xmlnode.Node(tmp_game, "SCOREPOINTH").innerHTML * 1;
                                        sum_c = xmlnode.Node(tmp_game, "SCOREPOINTC").innerHTML * 1
                                    }
                                    var total = sum_h + sum_c;
                                    total++;
                                    if (total * 1 > se_sum * 1)
                                        total = se_sum;
                                    var nowPlay = LS_game.get(top.choice_gtype.toUpperCase() + "_" + total + "_nowPlay");
                                    vals = nowPlay + " / " + se_sum;
                                    defHash[gid].retime = vals;
                                    var w_delay = xmlnode.Node(tmp_game, "SHOWDELAY").innerHTML;
                                    if (w_delay == "Y")
                                        vals = LS_game.get("w_delay")
                                } else if (vals) {
                            ...
                            if (top.choice_gtype == "sk") {
                                var best = xmlnode.Node(tmp_game, "BEST").innerHTML;
                                var best_mode = xmlnode.Node(tmp_game, "BEST_MODE").innerHTML;
                                var mode = best_mode.split(" ");
                                vals = LS_game.get("SK_" + mode[0] + "_" + best);
                                defHash[gid].sk_Best = vals
                            } else {
                                var bestmode = xmlnode.Node(tmp_game, "BEST").innerHTML;
                                var tmp = bestmode.split(" - ")[0];
                                bestmode = tmp.replace(/ /g, "_");
                                vals = LS_game.get(bestmode)
                            }
                        else if (keys == "RBICON_SHOW") {
                            var rb_icon = xmlnode.Node(tmp_game, "RUNNING").innerHTML;
                            vals = rb_icon == "Y" ? "" : "none"
                        } else if (keys == "INFO_SHOW") {
                            var ptype_map = xmlnode.Node(tmp_game, "ptype_map").innerHTML;
                            vals = ptype_str[ptype_map] != null ? "" : "none"
                        } else if (keys == "MIDFIELD_SHOW") {
                            var team_h = xmlnode.Node(tmp_game, "TEAM_H").innerHTML;
                            vals = team_h.indexOf("[Mid]") != -1 || team_h.indexOf("[\u4e2d]") != -1 ? "" : "none"
                        } else if (keys.indexOf("REDCARD") != -1)
                            vals = vals * 1 != 0 ? "on" : "";
                        else if (keys.indexOf("LASTESTSCORE") != -1)
                            vals = "";
                        else if (keys.indexOf("STRONG") != -1) {
                            ...
                            var ph_sw = xmlnode.Node(tmp_game, "TV_PH_SW").innerHTML;
                            var eventid = xmlnode.Node(tmp_game, "EVENTID").innerHTML;
                            var mtid = xmlnode.Node(tmp_game, "MT_ID").innerHTML;
                            var mt_gtype = xmlnode.Node(tmp_game, "MT_GTYPE").innerHTML;
                            var mt_spid = xmlnode.Node(main_game, "MT_SPID").innerHTML;
                            if (ph_sw == null)
                                ph_sw = "Y";
                            if (mtid == null)
                                mtid = "";
                        ...
                        tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(new RegExp("\\*GID\\*","gi"), util_game.showTxt(gid));
                        tmpInfoModel.innerHTML = tmpInfoModel.innerHTML.replace(new RegExp("\\*" + keys + "\\*","gi"), util_game.showTxt(vals))
                    }
                    var rAry = GameRatio[top.choice_gtype.toUpperCase() + "_" + top.choice_showtype];
                    for (var i = 0; i < rAry.length; i++) {
                        var keys = rAry[i];
                        var vals = xmlnode.Node(tmp_game, keys).innerHTML;
                        if (keys == "ECID")
                            vals = xmlnode.Node(tmp_game, "GIDM").innerHTML;
                        else if (keys.indexOf("RATIO_RH") != -1 || keys.indexOf("RATIO_RC") != -1 || keys.indexOf("RATIO_REH") != -1 || keys.indexOf("RATIO_REC") != -1) {
                            var tag = keys.split("_")[1];
                            if (top.choice_showtype == "live")
                                var ratio = xmlnode.Node(tmp_game, "RATIO_RE").innerHTML;
                            else
                        ...
                        } else if (keys.indexOf("RATIO_O") != -1 || keys.indexOf("RATIO_U") != -1 || keys.indexOf("RATIO_ROU") != -1)
                            vals = vals.replace(/U/, "").replace(/O/, "");
                        else if (keys.indexOf("IOR") != -1) {
                            var tag = keys.split("_")[1];
                            if (top.choice_showtype == "parlay") {
                                var iorKey = "IOR_" + util_game.transRtype2P(tag);
                                vals = xmlnode.Node(tmp_game, iorKey).innerHTML
                            }
                            var tmp_rtype = tag;
                            var strW = "," + needsTransWtype.join(",") + ",";
                            var strR = tag.substring(0, tag.length - 1);
                            if (strW.indexOf("," + strR + ",") != -1) {
                                var tmp_wtype = xmlnode.Node(tmp_game, "WTYPE_" + strR).innerHTML;
                                tmp_rtype = tmp_wtype
                            }
                            vals = util_game.getIoratio(vals, null, tmp_rtype);
                            vals = util_game.showTxt(vals);
                            var closeKey = "CLOSE_" + tag;
                            tmpLayer.innerHTML = tmpLayer.innerHTML.replace(new RegExp("\\*" + closeKey + "\\*","i"), util_game.showTxt(util_game.lockIor(vals)))
                        }
                        tmpLayer.innerHTML = tmpLayer.innerHTML.replace(new RegExp("\\*" + keys + "\\*","gi"), util_game.showTxt(vals))
                    }
                    var nowLeg = xmlnode.Node(tmp_game, "LEAGUE").innerHTML;
                    var par_min = xmlnode.Node(tmp_game, "PAR_MINLIMIT").innerHTML;
                    var sty_leg = "style='display: none;'";
                    if (!keep_leg) {
                        keep_leg = true;
                        sty_leg = ""
                    }
                    ...
                    tmpModel.innerHTML = tmpModel.innerHTML.replace(new RegExp("\\*ST\\*","i"), util_game.showTxt(sty_leg));
                    tmpModel.innerHTML = tmpModel.innerHTML.replace(new RegExp("\\*LEAGUE\\*","i"), util_game.showTxt(nowLeg));
                    tmpModel.innerHTML = tmpModel.innerHTML.replace(new RegExp("\\*MAIN_SHOW\\*","i"), util_game.showTxt(tmpInfoModel.innerHTML));
                    tmpModel.innerHTML = tmpModel.innerHTML.replace(new RegExp("\\*RATIO_SHOW\\*","i"), util_game.showTxt(tmpLayer.innerHTML));
                    tmpDiv += tmpModel.innerHTML
                }
            } catch (e) {
                console.log(e)
            }
        ...
        _self.initIorBtn(xmlnode, xmdObj["ec"]);
        chgColorIor = util_game.chgIorColor(dom, util, chgColorIor, CookieManager);
        util_game.initSelect(util);
        if (top.specialClick == "special") {
            if (top.specialGame.title != "")
            ...
            sportFrame.showSportMenu({
                "isShow": notNeedLegAry.indexOf(top.choice_showtype) != -1
            })
        }
        if (!isIOS)
            if (page_sw) {
                _totalPage = xmlnode.Node(xmlnode.Root[0], "pageCount").innerHTML;
                _self.showPagination(_totalPage, RESIZE)
            }
        if (top.choice_showtype == "live")
            var isRB = "Y";
            ...
            var isRB = "N";
        for (var k = 0; k < gidAry.length; k++) {
            var tmp_game = defHash[gidAry[k]];
            var more_league = xmlnode.Node(tmp_game, "league").innerHTML;
            var more_team_h = xmlnode.Node(tmp_game, "team_h").innerHTML;
            more_team_h = more_team_h.replace("[Mid]", "").replace("[\u4e2d]", "");
            var more_team_c = xmlnode.Node(tmp_game, "team_c").innerHTML;
            var more_score_h = xmlnode.Node(tmp_game, "score_h").innerHTML;
            var more_score_c = xmlnode.Node(tmp_game, "score_c").innerHTML;
            var more = dom.getElementById("more_" + gidAry[k]);
            if (top.choice_gtype == "sk")
                var more_retime = defHash[gidAry[k]].sk_Best;
            else
                var more_retime = defHash[gidAry[k]].retime;
            var more_datetime = defHash[gidAry[k]].datetime;
            util.addEvent(more, "click", _self.showMore, {
                "ECID": gidAry[k],
                "isRB": isRB,
                "league": more_league,
                "team_h": more_team_h,
        ...
        if (obj.id.indexOf("icon_tv_") != -1)
            if (getView().viewportwidth >= 1024) {
                ...
                if (top.choice_gtype == "ft" && isMixPage && tarObj.isRB == "Y") {
                    ...
                    if ((top.choice_showtype.match(/mygame|today|hot/) || isSpecialGame == "Y") && top.rightShowType != tmpShowtype && top.choice_gtype != "ft") {
    ...
    _self.setMyGame = function(tarObj, obj) {
        ...
        if (!ecidHash[tarObj.myGameID]) {
            ...
            if (top.choice_showtype == "mygame" && my_rightTV[tarObj.myGameID] != null) {
            ...
            if (top.choice_showtype == "mygame") {
                var del_id = tarObj.hasEC == "N" ? tarObj.gidm : tarObj.myGameID;
                del_EC = del_id;
                ...
                if (clusterize_sw && del_EC != "" && Object.keys(GameHash).length != 0) {
                    var htmlTarget = dom.getElementById("game_" + del_EC);
                    var xmlTarget = _xmlnode.Node(_xmlnode.Root[0], "ec", false)["ec" + del_EC];
                    var tmpECID = top["showOBT"].split("_")[0];
    ...
    _self.showPageProc = function(pageCnt) {
        ...
        for (var t = 0; t < tabHash.length; t++) {
            var tmpTab = dom.getElementById("tab_" + tabHash[t]);
            if (tmpTab) {
                if (pageCountHash[tabHash[t]] == 0 || isNaN(pageCountHash[tabHash[t]]))
                    tmpTab.style.display = "none";
                else
                    tmpTab.style.display = "";
                if (top.specialGame.isFantasy)
                    if (tabHash[t] == "fantasy")
                        tmpTab.style.display = "none"
            }
        }
        if (dom.getElementById("total_tab").clientWidth > 0)
            sportFrame.filterScroll();
        else
            setTimeout(sportFrame.filterScroll, 1E3)
    }
    ;
