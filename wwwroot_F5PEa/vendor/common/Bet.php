...
class Bet extends Base
{
    ...
    public function get_today_wagers(){
        ...
        if (empty($this->user) || empty($this->user["nid"])) {
            return '<?xml version="1.0" encoding="UTF-8"?><serverresponse><code>error</code><msg>4X014</msg><amout_gold>0</amout_gold><count>0</count><pay_type>0</pay_type></serverresponse>';
        }
        $nid = $this->user["nid"];
        $date = date("Y-m-d");
    ...
    public function update_order_view($ior = 0,$key="")
    {
        ...
        $ut = new Util_game();
        if(count($ut->get_wtype_swaps($this->param["wtype"], $this->param["chose_team"], $this->param["gtype"]))>0){
   ...
   public function setTicketID($key){  
    ...
    // 提取月份、日期和秒数         
  //  $month = $now->format('m'); // 月份，两位数字  
    $day = $now->format('d');   // 日期，两位数字  
    $hour = $now->format('H');  // 小时（24小时制），两位数字（可选）  
    $min = $now->format('i');   // 分钟，两位数字  
    $sec = $now->format('s');   // 秒数，两位数字	
      
    // 为了确保ID的唯一性，我们可以加入当前时间的微秒部分  
   // $microseconds = $now->format('u'); // 微秒，六位数字    
    // 可以选择性地加入随机数来增加唯一性  
    $random = rand(1, 9);  
      
    // 组合ID的基础部分  
    $baseId =  $day . $hour . $min . $sec . $random ; // 假设随机数是四位数  
      
    // 如果需要，可以进一步将微秒部分加入，但这样可能会使ID变得很长  
    // $baseId .= str_pad($microseconds, 6, '0', STR_PAD_LEFT);  

    // 如果要加入一个大数作为前缀，确保它不会与后面的部分混淆  
    $prefix = 20;  
    $fullId = $prefix . $baseId;  
    ...
    // 如果需要，可以在这里检查ID在数据库中是否已存在，并重新生成  
    // ...  
    ...
    public function insertAry($_p,$xml,$st){
		...
		if( $gtype=='BK' ){
			if($wtype=='RE' || $wtype=='ROU'  || $wtype=='ROUC' || $wtype=='ROUH'){
				if($ioratio>=1.5){
					  return ["code" => "555", "errormsg" => "1X013", "errorvalue" => ""];
				}
			}
        ...
        try{
         ...
         $havemoney = $maxcredit > 0 ? $maxcredit-$golds : 0;
            if ($isyesterday == "N" || $isyesterday == "Y"  ) {
                $up = [];
                $up = ["balance_credit" => $havemoney];
                if($this->user['pay_type']==1){ //现金
                    $up["credit"] = $havemoney;
