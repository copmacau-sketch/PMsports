...
def list_events(
    ...
    for row in rows:
        if _is_inplay_row(row):
            ...
            # Apply the same decided-market filter to the WS snapshot
            # so a row sourced from /dev/shm doesn't show a locked
            # market that the r_cn-sourced row would have hidden.
            if snap:
                snap_markets = snap.get("markets") or []
                snap_markets = _apply_decided_filters(row, snap_markets)
                snap = {**snap, "markets": snap_markets}
    ...
    try:
        ...
        with _logo_urlreq.urlopen(req, timeout=_LOGO_HTTP_TIMEOUT) as r:
            ...
            if not raw or "html" in ct:
                return None
    except (_logo_urlerr.URLError, _logo_urlerr.HTTPError, TimeoutError, OSError):
        return None
...
def logo_proxy(category: str, gid: int):
    ...
    if data is None or len(data) < 128:
        # Negative-cache: touch an empty file.
        try:
