...
class GoalserveTeamDirectory:
    ...
    def _poll_once(self) -> None:
        """One blocking fetch+parse.   Runs in a worker thread (see run).
        Exceptions are caught and logged; never re-raised so the loop
        keeps cycling on the next interval."""
        t0 = time.time()
        body = _fetch_xml(self.url)
        ...
        try:
            conn = _open_db()
            try:
                teams, leagues = _parse_into_db(body, conn)
            finally:
                conn.close()
        except sqlite3.Error as e:
        ...
        self.last_count = teams
        log.info(
            "td poll ok: %d teams + %d leagues parsed in %.2fs (bytes=%d)",
            teams, leagues, time.time() - t0, len(body),
        )
