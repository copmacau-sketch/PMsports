<?php
/**
 * DbBridge — 从 db_markets.foot_match + foot_match_xml 直接生成 SPA 兼容的 XML
 *
...
class DbBridge {
    ...
    /** 是否有任何 db_markets.foot_match 数据；transform.php 拦截前会先调它 */
    public function hasData(): bool {
        $row = $this->db->select("SELECT COUNT(*) AS c FROM db_markets.foot_match", "Row");
    ...
    public function getGameList(array $params): string {
        ...
        $sql = "SELECT m.gid, m.lid, m.league, m.team_h, m.team_c, m.datetime,
                       ...
                       x.r_cn
                FROM db_markets.foot_match m
                LEFT JOIN db_markets.foot_match_xml x ON x.gid = m.gid
                WHERE {$where}
        ...
        if ($ecid > 0) {
            $row = $this->db->select(
                "SELECT m.gid, m.team_h, m.team_c, x.r_cn
                 FROM db_markets.foot_match m
                 JOIN db_markets.foot_match_xml x ON x.gid = m.gid
