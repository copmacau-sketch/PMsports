...
util.isPad=function(){
if(util.isIPad() || util.isMIPad() || util.isMIPAD2() || util.isMI2() || util.isSamsungFlat()) return true;
return false;
}

util.isMIPad=function(){
return (top.http_user_agent.indexOf("MI PAD")!=-1)?true:false;
}

// Mozilla/5.0 (Linux; Android 5.1; MI PAD 2 Build/LMY47I) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.111 Safari/537.36
util.isMIPAD2=function(){
return (top.http_user_agent.indexOf("MI PAD 2")!=-1)?true:false;
}

// Mozilla/5.0 (Linux; Android 7.1.1; MIX  2 Build/NMF26X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.111 Safari/537.36
util.isMI2=function(){
return (top.http_user_agent.indexOf("MIX 2")!=-1)?true:false;
}

//三星平板
util.isSamsungFlat=function(){
return (top.http_user_agent.indexOf("Android 4.")!=-1&&top.http_user_agent.indexOf("SM-")!=-1)?true:false;
}
...
//EngNeed 至少需要幾個字母組成
util.str_chk = function (str, EngNeed) {
var str_char = 0;
...
var base_url = "";
var base_param = "";
var mail_forbid = false;
/*
在 2018/10/5，於 下午3:27，Roy 編輯:
> 3142.麻煩將線上199.26.96.204,205.201.0.204  dns頁面, 這二個ip打開的畫面幫預設在簡體
> >>>. try
>  有將IP修正成 199.26.100.169   205.201.4.169 麻煩在幫點點

不好意思  該ip預計下週會做調整, 麻煩幫改回en1的ip:203.160.140.94, 180.94.224.94
*/
//var vpn_ary = new Array("199.26.96.224","180.94.224.94"); // 故障疑難排解，固定IP
var vpn_ary = "vpn.hga030.com";//2021-0226 改為疑難排解的Domain vpn.hga030.com

function init(){
trace("init");
if(opener==null){
//top.location.href = util.getNowDomain();
return;
}

if(top.uid=="undefined")top.uid = opener.top.uid;
if(top.langx=="undefined")top.langx = opener.top.langx;
top.account_type = "";
top.showResultObj=new Object();
top.showFTResultObj=new Object();
if(top.mem_status=="undefined")top.mem_status = opener.top.mem_status;
if(top.showKR=="undefined")top.showKR = opener.top.showKR;
//Ricky 2019-02-14 另開頁的裝置
if(top.http_user_agent=="undefined")top.http_user_agent = opener.top.http_user_agent;
//var org_type = opener.top.account_type;
var org_type = "result";
base_url = util.getNowDomain()+"/app/member/account/";
base_param = "uid="+top.uid+"&langx="+top.langx;
var param = base_param+"&type="+org_type;

_mc["body"] = document.getElementById("body");
_mc["order"] = document.getElementById("order");
_mc["top_div"] = document.getElementById("ACC_W");

setTopWidth(_mc["top_div"]);
goLink(org_type);
iframe_src(_mc["order"], base_url+"myaccount.php?"+param);

}

function setTopWidth(obj){
var cls = (util.isIPad())?"index_accMainPAD":"index_accMainTB";
util.setObjectClass(obj, cls);
}

function linkEvent(_id){
_mc["order"].contentWindow.chgClickStatus(_id);
goLink(_id);
}
...
var tmp_langx = (top.langx=="ko-kr")?"en-us":top.langx;
// 2018-08-13 在 2018/8/13，於 下午3:28，阿仁(Aaron) 編輯:  程式可以把180.94.224.94 改成vpn.hg0088.com嗎
// var _url = "http://180.94.224.94/tpl/member/"+tmp_langx+"/index.html";

//var _url = "http://vpn.hg0088.com/tpl/member/"+top.langx+"/index.html";
// 2018-10-05 3142.麻煩將線上199.26.96.204,205.201.0.204  dns頁面, 這二個ip打開的畫面幫預設在簡體
//var rd = getRandomInt(2);
var _url = "http://"+vpn_ary+"/tpl/member/"+tmp_langx+"/index.html";

var par = "width=900,height=650,status=no,location=no";
try{
if(!top.newWinObj["troubleshooting"]){
top.newWinObj["troubleshooting"] = window.open(_url, "troubleshooting", par);
}
}catch(e){
try{
top.newWinObj["troubleshooting"] = window.open(_url, "troubleshooting", par);
...
try{
top.newWinObj["troubleshooting"].focus();
}catch(e){}
return;
}

var tmpType = new Object();
tmpType[type] = type;
tmpType[type+"_param"] = "&type="+type;

tmpType["result"] = type+"/"+type;
tmpType["result_param"] = "&game_type=FT";
//if(top.account_type==type) return;

top.account_type = type;
var url = base_url+tmpType[type]+".php?";
var param = base_param+tmpType[type+"_param"];

if(tmpType[type] == "scroll_history"){
param += "&t_important="+(opener.top.t_important||1);
}
iframe_src(_mc["body"], url+param);
}

function chgPageEvent(filename){
if(!filename) return;
//iframe_src(_mc["body"], util.getNowDomain()+"/tpl/member/"+top.langx+"/rules_"+filename+".html");
iframe_src(_mc["body"], util.getNowDomain()+"/app/member/account/getRules.php?uid="+top.uid+"&filename="+filename+"&langx="+top.langx);
}

function systemMsg(msg){
util.systemMsg("[myaccount_index.js]"+msg);
}

function trace(msg){
util.trace("[myaccount_index.js]"+msg);
}

window.onunload=function(){
if(opener) opener.top.newWinObj["account"] = null;
}

function setAnnouCount(){
window.opener.getAnnouCount();
}
