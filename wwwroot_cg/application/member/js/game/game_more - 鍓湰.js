 function game_more(_win, _dom, _post) {
                ...
                _self.checkIntoRB = function(xmlnode, mainGame) {
                    ...
                    var live = util.getKeyValue(xmlnode, mainGame, "Live");
                    if (!top.go_to_rb)
                                        ...
                                        if (wtype == "SFS") {
                                            ...
                                            if (top.choice_gtype == "ft" && util_game.checkWtypeIsPD(wtype)) {
                                                ...
                                                if (wtype == "PD" || wtype == "RPD") {
                                                    ...
                                                    if (!rtypes) {
                                                        console.log(wtype + ">>pdIorHead[" + ET_str + "ec" + ecid + "]:", rtypes);
                                                        continue
                                                    }
                                                } else if (wtype == "HPD" || wtype == "HRPD") {
                                                    rtypes = pdIorHead[ET_str + "ec" + ecid + "_H"];
                                                    if (!rtypes) {
                                                        console.log(wtype + ">>pdIorHead[" + ET_str + "ec" + ecid + "_H]:", rtypes);
                                                        continue
                                                    }
                                                }
                                            }
                ...
                _self.parseJsonData = function(param, lastWtypeHeight) {
                    ...
                    if (gidHash) {
                        ...
                        for (var a = 0; a < wHash.length; a++) {
                            ...
                            if ((top.choice_gtype.match(/ft|es/) || top.choice_gtype == "bk" && showtype != "live") && clickHeadfilter != "Main")
                                if (!pageFilterHash[clickHeadfilter].includes(wHash[a]))
                                    continue;
                            var tmp = wHash[a].split("_");
                            var gameType = "";
                                ...
                                for (var i = 0; i < gidHash[_mode].length; i++) {
                                    ...
                                    if (gObj != null) {
                                        ...
                                        if (period != "" && period != period_id)
                                            continue;
                                        sw_wtype = gObj[wtypeStr]["SW_" + wtypeForSW.toUpperCase()];
                                        ...
                                        var important = gObj["IMPORTANT"];
                                        var team_h = gObj["TEAM_H"];
                                        var team_c = gObj["TEAM_C"];
                                        var hgid = gObj["HGID"];
                                        var gidm = gObj["GIDM"];
                                        var session = gObj["SESSION"];
                                        var tmp_gid = isHalf ? hgid : gid;
                                        var str_gid = isHalf ? "HGID" : "GID";
                                        var half = isHalf ? "Y" : "N";
                                        team_h = isFilterPtype ? _self.transTeam(team_h, ptype, important) : team_h;
                                        ...
                                        if (wtype == "SFS") {
                                            ...
                                            for (var k = 0; k < rtypes.length; k++) {
                                                ...
                                                if (strRatio != "") {
                                                    var ratio = gObj[wtypeStr][strRatio.toUpperCase()];
                                                    if (wtype != "W3" && util_game.checkWtypeIsR(wtype)) {
                                                        var strStrong = !isHalf ? "STRONG" : "HSTRONG";
                                                ...
                                                if (showtype == "parlay") {
                                                    ...
                                                    var ior = gObj[wtypeStr]["IOR_" + rtypeStr]
                                                } else {
                                                    var ior = gObj[wtypeStr]["IOR_" + rtype];
                                                    ior = _self.chgOddIorJson(gObj, wtype, rtype, ior, wtypeStr)
                                                }
                                                if (isMutiType)
                                                    ior_wtype = wtype.substr(0, wtype.length - 1);
                                                else if (top.choice_gtype == "ft" && wtype == "RC")
                                                ...
                                                if (!util_game.checkWtypeIsPD(wtype)) {
                                                    ...
                                                    if (other_rtype != null) {
                                                        if (showtype == "parlay")
                                                            var other_ior = gObj[wtypeStr]["ior_" + util_game.transRtype2P(other_rtype)];
                                                        else
                                                            var other_ior = gObj[wtypeStr]["ior_" + other_rtype];
                                                        if (other_rtype * 1 == 0 || self_ior * 1 == 0)
                                                            ;
                                                        else
                                                            all_close = false
