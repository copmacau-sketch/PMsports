...
function getGameList(_source) {
	...
	for (var x = _start - 1; x < _end; x++) {
		...
		if (total_parlay_limit == 0 || total_parlay_limit <
			par_min) total_parlay_limit = par_min;
		var tmpNowModel = "";
		...
		if (_top.choice_rtype.match(/pd/)) {
			var isMix = myGameRtype != "" && myGameRtype != "rb" && !_top.isFantasyPage;
			strongMODEL =
				isHalf == "N" ? tmp_game["pd_strong"] : tmp_game["hpd_strong"];
			var tmpStr = "DRAW";
		...
		if (_top.choice_rtype.match(/pd/)) {
			...
			var showHT = "";
			if (tmp_game["is_rb"] == "Y") showHT = tmp_game["hnike"] == "N" || tmp_game["hgopen"] ==
				"N" || tmp_game["hpd_sw"] == "N" ? "none" : "";
			...
			if (isHalf == "Y") tmpLayer = tmpLayer.replace(new RegExp("\\*GID\\*",
				"gi"), tmp_game["hgid"]);
			if (nowPDMode == "all") {
			...
			if (pdSortHash[_key + halfStr]["All"].length >
				0) tmpLayer = getPDModel(pdOBJ);
		...
		if (myLeg[leg_id] == null) myLeg[leg_id] =
			new Array;
		if (_top.choice_gtype == "ft" && (headertype == "league" && filterLid != "" || _top.isFantasyPage &&
			...
			if (LidPDisopen == keepLegID + "_true" || tmp_removeLeg == "") {
				...
				if (notShowLegCount == 0 && totalLeg.length <=
					10 || notShowLegCount > 0 && notShowLeg[keepLegID] == false) isShowLegGame = true;
			...
			if (last_league_header == leg_id && tmp_removeLeg != "") {
				...
				tmpModel = tmpModel.replace(/\*FANTASTY_LID\*/i, now_lid);
				if (notShowLegCount == 0 && totalLeg.length <= 10 || notShowLegCount > 0 && notShowLeg[now_lid] ==
					false) isShowLegGame = true;
				if (!notShowLegGame[ECID] && isShowLegGame) tmpHeight += DEFINED_ROWHEIGHT["GAMEBORDER_FIX"]
			} else {
				var tmpLeg_id =
					leg_id + "_" + sameLegCount[leg_id];
		...
		if (notShowLegCount == 0 && totalLeg.length <= 10 || notShowLegCount > 0 && notShowLeg[now_lid] == false)
			isShowLegGame = true;
		...
		if (_top.choice_showtype == "parlay")
			...
			else if (TAB_ary.indexOf(_top.choice_rtype) != -1) tmpRtype = myGameRtype !=
			"rb" ? _top.choice_rtype : "r" + _top.choice_rtype;
		else tmpRtype = myGameRtype != "rb" ? "r" : "rb";
		else if (_isMyGame == "mygame")
			if (hasPD) tmpRtype = myGameRtype != "rb" ? "pd" : "rpd";
		...
		if (isFantasy == "Y") {
			...
			var GameH =
				new Array("GAMEH_TEAM_C_ID", "GAMEH_TEAM_H_ID");
			for (var f = 0; f < fantasyGameHead.length; f++)
				if (fantasyObj[_key] != null) {
					...
					tmpFantasyInfoModel =
						tmpFantasyInfoModel.replace(new RegExp("\\*" + tmpKey + "\\*", "gi"), showTxt(vals))
		...
		Score["ET_C"] =
			tmp_game["et_scroe_c"] != "" && tmp_game["et_scroe_c"] * 1 >= 0 ? tmp_game["et_scroe_c"] : 0;
		var ScoreH = Score["FT_H"] * 1 + Score["ET_H"] * 1;
		var ScoreC = Score["FT_C"] * 1 + Score["ET_C"] * 1;
		...
		for (var i = 0; i < InfoAry_length; i++) {
			...
			if (keys == "ECID") {
				...
				if (_top.choice_showtype == "parlay" && is_rb != "Y") {
					var _datetime = tmp_game["datetime"];
					var sys_time = tmp_game["systime"];
					var obj = {
						"datetime": _datetime,
				...
				var tag =
					keys.split("_")[2].toLowerCase();
				var red_count = tmp_game["redcard_" + tag];
				vals = red_count * 1 != 0 ? "red_" + red_count : ""
			} else if (keys.indexOf("LASTESTSCORE") != -1) vals = vals != "" ? "last_goal" : "";
		...
		if (tmp_game["sfsgame"] && SFSObj[_key]) {
			...
			if (max_FS > 0) {
				for (var i = 0; i < max_FS; i++) {
					...
					for (var keys in S_LIST) {
						...
						tmp_model_sfs_game = tmp_model_sfs_game.replace(new RegExp("\\*ECID\\*", "gi"), util_game
							.showTxt(ECID));
						tmp_model_sfs_game = tmp_model_sfs_game.replace(new RegExp("\\*SFS_TEAM_NAME_" +
							HC + "\\*", "gi"), util_game.showTxt(tmp_SFS_NAME));
					...
					for (var _type in noIorData)
						if (noIorData[_type]) tmp_model_sfs_game =
							tmp_model_sfs_game.replace(new RegExp("\\*NOIORDATA_" + _type + "\\*", "gi"), "none");
					tmpSFSModel += tmp_model_sfs_game
				}
				if (now_lid != "" && tmp_removeLeg == now_lid && totalLeg.indexOf(now_lid) == -1) {
				...
				if (LegHeaderExist == "" && !(LidPDisopen == now_lid + "_true") && now_lid != "" && totalLeg.indexOf(
						now_lid) != -1 && reJointotalLeg.indexOf(now_lid) == -1) {
					totalLeg.splice(totalLeg.indexOf(now_lid),
						1);
					tmp_removeLeg = now_lid;
					if (delLidAry.indexOf(now_lid) == -1) delLidAry.push(now_lid)
				} else if (headertype == "league" && filterLid != "" && delLidAry.indexOf(filterLid) == -1)
				...
				if (nowType == "mother" || nowType != "mother" && tmp_game[tmpType + "gid"]) {
					if (_top.choice_rtype.match(/pd/))
						...
						if (LegHeaderExist == "" && !(LidPDisopen == now_lid + "_true") && now_lid != "" && totalLeg
							.indexOf(now_lid) != -1 && reJointotalLeg.indexOf(now_lid) ==
							-1) {
							totalLeg.splice(totalLeg.indexOf(now_lid), 1);
							tmp_removeLeg = now_lid;
							if (delLidAry.indexOf(now_lid) == -1) delLidAry.push(now_lid)
					...
					for (var i = 0; i < rAry.length; i++) {
						...
						var tmpkeys = keys;
						if (keys.match(/^RATIO_H?RE?[H|C]$/g)) tmpkeys = keys.substring(0, keys.length -
							1);
						if (nowType != "mother")
						...
						if (keys == "ECID" && vals == "") vals = tmp_game["gidm"];
							...
							if (keys.indexOf("RATIO_H") != -1) var _key = nowType == "mother" ? "HSTRONG_" + tmp_game[
									"hstrong"] : nowType.toUpperCase() +
								"HSTRONG_" + tmp_game[tmpType + "hstrong"];
							...
							if (zeroCloseRtype.indexOf(_top.choice_rtype) != -1)
								if (!PD_open)
									if (LegHeaderExist == "" && !(LidPDisopen == now_lid +
											"_true") && now_lid != "" && totalLeg.indexOf(now_lid) != -1 &&
										reJointotalLeg.indexOf(now_lid) == -1) {
										totalLeg.splice(totalLeg.indexOf(now_lid), 1);
										tmp_removeLeg = now_lid;
										if (delLidAry.indexOf(now_lid) == -1) delLidAry.push(now_lid)
									} else {
							...
							if (vals * 1 == -99) {
								...
								tmpLayer = tmpLayer.replace(new RegExp("\\*" + tmpWtype +
									"_" + nowType.toUpperCase() + "BLANK\\*", "gi"), "odd_empty")
							} else tmpLayer = tmpLayer.replace(new RegExp("\\*" + closeKey + "\\*", "i"), util_game
								.lockIor(vals))
						}
						if (nowType == "mother") tmpLayer = tmpLayer.replace(new RegExp("\\*" + keys + "\\*", "gi"),
							showTxt(vals));
			...
			if (rtypeLoop.indexOf(tmpRtype) !=
				...
				for (var key in tmp_game)
					if (key.match("sub")) {
						...
						if (tmpSubHash[game_type] == null) tmpSubHash[game_type] = new Object;
		...
		if (hasEC == "Y") {
			...
			if (tmp_game["wi_count"] &&
				tmp_game["wi_count"] * 1 != 0) tmpOBTModel = tmpOBTModel.replace(/\*OBT_WI_STR\*/i, LS_game.get(
				"OBT_WI"));
			...
			tmpOBTModel = tmpOBTModel.replace(/\*BLOCKSCROLL\*/i, !isIOS ? "update" : "");
		...
		tmpModel =
			tmpModel.replace(/\*PARLAY_MIN\*/i, par_min);
		...
		tmpModel = tmpModel.replace(/\*OBT_SHOW\*/i, filterRtypeAry.indexOf(_top.choice_rtype) == -1 && hasEC == "Y" &&
			...
			tmpOBTModel : "");
		tmpModel =
			tmpModel.replace(/\*DIS_PEN\*/i, nowModel == "PK" ? "" : "style='display:none;'");
...
function getOthersGameList(_source) {
	...
	var viewport_height = _source["viewport_height"];
	var DEFINED_ROWHEIGHT = _source["DEFINED_ROWHEIGHT"];
	var CLUSTERIZE_LIMIT_S =
		_source["CLUSTERIZE_LIMIT_S"];
	var CLUSTERIZE_LIMIT_M = _source["CLUSTERIZE_LIMIT_M"];
	var CLUSTERIZE_LIMIT_L = _source["CLUSTERIZE_LIMIT_L"];
	var CLUSTERIZE_SW = _source["CLUSTERIZE_SW"];
	var pageIndex = 0;
	...
	for (var x = _start - 1; x < _end; x++) {
		...
		var leg_id = tmp_game["lid"];
		var flag_class = tmp_game["flag_class"] != null ? tmp_game["flag_class"] : "flag_BS";
		if (total_parlay_limit ==
			0 || total_parlay_limit < par_min) total_parlay_limit = par_min;
		nowModel = tmp_game["now_model"];
		...
		' style="display: none;"');
			...
			if (!util.in_array(ECID,
					myLeg[keepLegID])) myLeg[keepLegID].push(ECID);
			tmpModel = tmpModel.replace(/\*ST\*/i, " id='LEG_" + leg_id + "' style='display: none;'");
			if (notShowLegCount == 0 && totalLeg.length <= 10 || notShowLegCount > 0 && notShowLeg[keepLegID] == false)
				isShowLegGame = true;
			if (!notShowLegGame[ECID] && isShowLegGame) tmpHeight += DEFINED_ROWHEIGHT["GAMEBORDER_FIX"]
		} else if (nowLeg != keepLeg && !util.in_array(leg_id, totalLeg)) {
			sameLegCount[leg_id] = 1;
			...
			if (!util.in_array(ECID, myLeg[tmpLeg_id])) myLeg[tmpLeg_id].push(ECID);
			keepLegID = tmpLeg_id;
			tmpModel = tmpModel.replace(/\*ST\*/i, " id='LEG_" +
				tmpLeg_id + "'");
			if (x != 0) tmpHeight += DEFINED_ROWHEIGHT["LEAGUE_FIX"] + DEFINED_ROWHEIGHT["LEAGUEBORDER_FIX"];
			sameLegCount[leg_id]++
		}
		...
		for (var i = 0; i < InfoAry_length; i++) {
			...
			if (keys == "ECID") vals = tmp_game["gidm"];
				...
				var STYLE_TOP_BOT = "";
				if (gameRound[0].toUpperCase() == "TOP") STYLE_TOP_BOT =
					"icon_bs_up";
				...
				tmpInfoModel = tmpInfoModel.replace(/\*SERVE_C\*/gi, point_tag == "Top" || point_tag == "Straight" ?
					"on" : "")
				...
				if (_top.choice_showtype == "live" || (_top.choice_showtype == "mygame" || _top.specialClick != "") &&
					myGameRtype == "rb")
					if (typeShow == "bk") {
						...
						if (sw_3x3 == "Y" && rb_time != "") vals = rb_time;
			...
			} else if (keys.indexOf("LASTESTSCORE") != -1) vals = "";
			else if (keys.indexOf("STRONG") != -1) {
				var tag = keys.split("_")[1];
				var strong = tmp_game["strong"];
		...
		for (var i = 0; i < rAry_length; i++) {
			...
			if (keys == "ECID") vals = tmp_game["gidm"];
				...
				if (_top.choice_gtype == "bk") vals = LS_game.get("BK_score_" +
					_ms + "_outer");
				else {
					var upperCaseGtype = _top.choice_gtype.toUpperCase();
					if (_top.choice_gtype.match(/bm|tn|tt|vb/)) vals = LS_game.get(upperCaseGtype + "_game_" + _ms +
						"_set_outer");
			...
			} else if (keys.indexOf("IOR") !=
				-1) {
				var tag = "";
				var HForMS = "";
				if (keys.split("_")[0].match(/HALF|MS|POINT/)) {
					tag = keys.split("_")[2];
					HForMS = keys.split("_")[0] + "_"
				...
				vals = showTxt(vals);
				if (!PD_open && hasPD)
					if (totalLeg.indexOf(now_lid) !=
						-1) totalLeg.splice(totalLeg.indexOf(now_lid), 1);
				var closeKey = "CLOSE_" + HForMS + tag;
				tmpLayer = tmpLayer.replace(new RegExp("\\*" + closeKey + "\\*", "i"), util_game.lockIor(vals))
			}
...
function getCupGameList(_source) {
	...
	model_HT_R_moua = _source["model_HT_R_moua"];
	model_FT_R_moua =
		_source["model_FT_R_moua"];
	model_HT_HOST = _source["model_HT_HOST"];
	model_HT_DRAW = _source["model_HT_DRAW"];
	model_HT_CUSTOMER = _source["model_HT_CUSTOMER"];
	model_FT_HOST = _source["model_FT_HOST"];
	...
	tmpHeight += DEFINED_ROWHEIGHT["LEAGUE_FIX"];
	if (viewport_height <=
		600) _BLOCK_LIMIT_HEIGHT = CLUSTERIZE_LIMIT_S;
	else if (viewport_height > 600 && viewport_height <= 900) _BLOCK_LIMIT_HEIGHT = CLUSTERIZE_LIMIT_M;
	else if (viewport_height > 900) _BLOCK_LIMIT_HEIGHT = CLUSTERIZE_LIMIT_L;
	...
	for (var x =
		...
		if (isHL && filterRtypeAry.indexOf(_top.choice_rtype) == -1 && cup_MainLid == leg_id) {
			if (period == "PRE")
				...
				if (groupMaxCount == 2) lastGroupGid[group_id] =
					tmp_game["ecid"];
				groupMaxCount++;
		...
		var sfsOpen = false;
		var now_lid = "";
		var tmpModel, tmpInfoModel, tmpOBTModel, tmpGROUPModel, tmpLayer, tmpRightInfoModel, tmpFantasyInfoModel;
		var _gid = tmp_game["gid"];
		var is_rb = tmp_game["is_rb"];
		var isFantasy = tmp_game["isfantasy"];
		var isShowLegGame = false;
		...
		if (_top.choice_rtype.match(/pd/)) {
			var isMix = myGameRtype != "" && myGameRtype != "rb";
			strongMODEL = isHalf == "N" ?
				tmp_game["pd_strong"] : tmp_game["hpd_strong"];
			var tmpStr = "DRAW";
			if (strongMODEL != "N") tmpStr = strongMODEL == "H" ? "HOST" : "CUSTOMER";
			if (nowPDMode == "choice") tmpStr = "CHOOSE";
			if (isHalf == "N")
		...
		if (_top.choice_rtype.match(/pd/)) {
			...
			var tmpShow = "SHOW_" + tmpHalfStr;
			var showHT;
			if (tmp_game["is_rb"] == "Y") showHT =
				tmp_game["hnike"] == "N" || tmp_game["hgopen"] == "N" || tmp_game["hpd_sw"] == "N" ? "none" : "";
			else showHT = tmp_game["hgopen"] == "N" || tmp_game["hpd_sw"] == "N" ? "none" : "";
			var showFT = tmp_game["pd_sw"] == "N" ? "none" : "";
			var pdOBJ = new Object;
			var nowZero = isHalf == "Y" ? tmp_game["ht_allzero"] : tmp_game["ft_allzero"];
			...
			tmpModel = tmpModel.replace(new RegExp("\\*FT_SW\\*", "gi"), showFT);
			if (isHalf ==
				"Y") tmpLayer = tmpLayer.replace(new RegExp("\\*GID\\*", "gi"), tmp_game["hgid"]);
			if (nowPDMode == "all") {
				if (showMoreStatus == "Y") tmpLayer = tmpLayer.replace(new RegExp("\\*SHOWMORE\\*", "gi"), "on");
				if (nowZero == "Y") tmpLayer = tmpLayer.replace(new RegExp("\\*ALLZERO\\*", "gi"), "no_event_pd")
			} else if (nowZero == "Y") tmpLayer = tmpLayer.replace(new RegExp("\\*ALLCLOSE\\*", "gi"), "lock");
			...
			if (pdSortHash[_key + halfStr]["All"].length > 0) tmpLayer = getPDModel(pdOBJ);
			else if (LegHeaderExist == "" && !(LidPDisopen == now_lid + "_true") && now_lid != "" && totalLeg.indexOf(
					now_lid) != -1 && reJointotalLeg.indexOf(now_lid) == -1) {
				totalLeg.splice(totalLeg.indexOf(now_lid), 1);
				tmp_removeLeg = now_lid;
		...
		} else if (myGameRtype == "rb" || myGameRtype == "ft") lastDate = "today";
		else lastDate = nowDate;
		var sameLeg = false;
		var same_in = period == "IN" && nowLeg == keepLeg;
		var same_pre = period == "PRE" && nowLeg == keepLeg;
		if (same_in || same_pre) sameLeg =
			true;
		...
		if (_top.choice_gtype == "ft" && (headertype == "league" && filterLid != "" || lastDate == "first")) {
			...
			if (nowLeg == "" && keepLeg == "" && firstEmpty) {
				...
				tmpModel = tmpModel.replace(/\*add_margin_ST\*/i,
					header_css);
				tmpModel = tmpModel.replace(/\*FANTASTY_LID\*/i, keepLegID);
				if (notShowLegCount == 0 && totalLeg.length <= 10 || notShowLegCount > 0 && notShowLeg[keepLegID] ==
					false) isShowLegGame = true;
				if (!notShowLegGame[ECID] && isShowLegGame) tmpHeight += DEFINED_ROWHEIGHT["GAMEBORDER_FIX"]
			} else {
				now_lid = tmp_removeLeg;
		...
		} else if (nowLeg != keepLeg && !util.in_array(leg_id,
				totalLeg)) {
			sameLegCount[leg_id] = 1;
			LidPDisopen = leg_id;
			totalLeg.push(leg_id);
			...
			var last_league_header =
				totalLeg[totalLeg.length - 1].split("_")[0];
			if (last_league_header == leg_id && tmp_removeLeg != "") {
				sameLegCount[leg_id]--;
				if (sameLegCount[leg_id] == 0) {
					now_lid = leg_id;
		...
		if (now_lid == "") now_lid = keepLegID;
		if (notShowLegCount == 0 && totalLeg.length <=
			10 || notShowLegCount > 0 && notShowLeg[now_lid] == false) isShowLegGame = true;
		if (!notShowLegGame[ECID] && isShowLegGame) tmpHeight += DEFINED_ROWHEIGHT["GAME_FIX"];
		if (nowModel == "PK") {
			var _wtype = tmp_game["nowset"];
			var endGame = tmp_game["end_game"];
		...
		if (period == "PRE" && cup_MainLid == leg_id && isHL && !sameGroup) {
			...
			tmpHeight += DEFINED_ROWHEIGHT["CUP_DATE"]
				...
				"PRE" && sameGroup || period == "IN" &&
				sameDate && sameLeg) || cup_MainLid != leg_id && sameDate && sameLeg) tmpModel = tmpModel.replace(
			/\*CUP_DIS\*/i, ' style="display: none;"');
		else {
			var _datetime = tmp_game["datetime"];
			var sys_time = tmp_game["systime"];
			var gameDateObj = {
		...
		if (_top.choice_rtype.match(/rnou|sfs|pd|moua|cn|rn/)) tmpModel =
			tmpModel.replace(/\*BET_TYPE_OPTION\*/i, _top.choice_rtype);
		else tmpModel = tmpModel.replace(/\*BET_TYPE_OPTION\*/i, "8");
		...
		if (isFantasy == "Y") {
			...
			var team_h_id = tmp_game["team_h_id"];
			var GameC = new Array("GAMEC_TEAM_C_ID", "GAMEC_TEAM_H_ID");
			var GameH = new Array("GAMEH_TEAM_C_ID",
				"GAMEH_TEAM_H_ID");
			for (var f = 0; f < fantasyGameHead.length; f++)
				if (fantasyObj[_key] != null) {
					var tmpKey = fantasyGameHead[f];
		...
		Score["FT_C"] = tmp_game["ft_scroe_c"] != "" && tmp_game["ft_scroe_c"] * 1 >= 0 ? tmp_game["ft_scroe_c"] : 0;
		Score["ET_H"] = tmp_game["et_scroe_h"] != "" && tmp_game["et_scroe_h"] * 1 >= 0 ? tmp_game["et_scroe_h"] : 0;
		Score["ET_C"] = tmp_game["et_scroe_c"] !=
			"" && tmp_game["et_scroe_c"] * 1 >= 0 ? tmp_game["et_scroe_c"] : 0;
		var ScoreH = Score["FT_H"] * 1 + Score["ET_H"] * 1;
		var ScoreC = Score["FT_C"] * 1 + Score["ET_C"] * 1;
		var noKeyFTResult = tmp_game["ft_scroe_h"] == "" && tmp_game["ft_scroe_c"] == "";
		var noKeyETResult = tmp_game["et_scroe_h"] == "" && tmp_game["et_scroe_c"] == "";
		for (var i = 0; i < InfoAry_length; i++) {
			...
			if (keys == "ECID") {
				...
				var tmpScore = splitKey[2] == "H" ? ScoreH : ScoreC;
				if (vals != "" && vals * 1 >= 0 && (!noKeyFTResult ||
						!noKeyETResult))
					if (nowModel == "ET" && splitKey[0] == "FT") {
						keys = splitKey[1] + "_" + splitKey[2] + "_PAST";
						vals = "(" + vals + ")"
		...
		if (tmp_game["sfsgame"] && SFSObj[_key]) {
				...
				for (var i = 0; i < max_FS; i++) {
					...
					for (var _type in noIorData)
						if (noIorData[_type]) {
							...
							tmp_model_sfs_game = tmp_model_sfs_game.replace(new RegExp("\\*NOIORDATA_" +
								_type + "\\*", "gi"), "none")
						} tmpSFSModel += tmp_model_sfs_game
				}
				if (now_lid != "" && tmp_removeLeg == now_lid && totalLeg.indexOf(now_lid) == -1) {
					...
					if (headertype == "league" && filterLid != "" && delLidAry.indexOf(filterLid) != -1) {
						tmp_removeLeg = filterLid;
						delLidAry.splice(delLidAry.indexOf(filterLid),
							1)
					}
				}
				tmpModel = tmpModel.replace(/\*CHOSETEAM\*/i, sfsChoseTeam[_key]);
			...
			for (var k = 0; k < keyAry.length; k++) {
				...
				if (nowType != "mother") tmpType = nowType + "_sub_";
				if (tmpType !=
					"" && tmp_game["now_model"] == "PK") tmpType = "pk" + tmpType;
				if (rtypeLoop.indexOf(tmpRtype) == -1 && nowType != "mother") continue;
				if (nowType == "mother" || nowType != "mother" && tmp_game[tmpType + "gid"]) {
					if (_top.choice_rtype.match(/pd/))
						if (tmpPDGameRatio.length > 0) rAry = rAry.concat(tmpPDGameRatio);
					...
					for (var i = 0; i < rAry.length; i++) {
						...
						if (keys == "ECID" && vals == "") vals = tmp_game["gidm"];
							...
							if (zeroCloseRtype.indexOf(_top.choice_rtype) != -1)
								if (!PD_open)
									if (LegHeaderExist == "" && !(LidPDisopen == now_lid + "_true") && now_lid != "" &&
										totalLeg.indexOf(now_lid) !=
										-1 && reJointotalLeg.indexOf(now_lid) == -1) {
										totalLeg.splice(totalLeg.indexOf(now_lid), 1);
										tmp_removeLeg = now_lid;
										if (delLidAry.indexOf(now_lid) == -1) delLidAry.push(now_lid)
									} else {
								...
								tmp_removeLeg = "";
								if (delLidAry.indexOf(now_lid) !=
									-1) delLidAry.splice(delLidAry.indexOf(now_lid), 1)
							} else {
								if (filterLid && filterLid.indexOf(",") == -1) isFilterSelectOne = true;
								if (headertype == "league" && filterLid != "" && delLidAry.indexOf(filterLid) != -1) {
									tmp_removeLeg = filterLid;
							...
							if (vals * 1 == -99) {
								...
								tmpLayer.replace(new RegExp("\\*" + closeKey + "\\*", "i"), util_game.lockIor(vals))
						}
						if (nowType == "mother") tmpLayer = tmpLayer.replace(new RegExp("\\*" + keys + "\\*", "gi"),
							showTxt(vals));
						else tmpLayer = tmpLayer.replace(new RegExp("\\*" + nowType.toUpperCase() + keys + "\\*", "gi"),
		...
		if (hasEC == "Y") {
			...
			else tmpOBTModel = tmpOBTModel.replace(/\*OBT_WI_STR\*/i,
				LS_game.get("OBT_TQ"));
			tmpOBTModel = tmpOBTModel.replace(/\*ECID\*/gi, ECID);
			if (_top["showOBT"] != null && _top["showOBT"] != "")
				if (_lastOBT_div != "" && ECID == _lastOBT_div_ECID) {
			...
			else tmpOBTModel = tmpOBTModel.replace(/\*OBT_CLOSE_SHOW\*/i,
				"style='display:none'");
			tmpOBTModel = tmpOBTModel.replace(/\*BLOCKSCROLL\*/i, !isIOS ? "update" : "");
			tmpOBTModel = tmpOBTModel.replace(/\*OBT_DIS\*/i, OBT_dis)
		}
			...
			if (cup_standings_sw == "Y" && tmp_game["ecid"] == lastGroupGid[group_id] && util.in_array(group_id,
				...
				if (parti_len < 4) tmpModel = tmpModel.replace(/\*GROUP_SHOW\*/i, "");
					...
					for (var i = 0; i < parti_len; i++) {
						...
						var comAry = new Array("played",
							"win", "loss", "draw", "goals_for", "goals_against", "goals_diff", "points");
						...
						group_model_team = group_model_team.replace(/\*TEAMID\*/g,
							showTxt(team_id));
						group_model_team = group_model_team.replace(/\*TEAMGROUP\*/g, util.showTxt(group_id));
						...
						for (var _key in partiObj)
							...
							for (var c = 0; c < tmplen; c++) {
								...
								group_model_body = group_model_body.replace(new RegExp("\\*GID_" + rtypeName[tmpCnt] +
									"\\*", "gi"), showTxt(gid));
								group_model_body =
									group_model_body.replace(new RegExp("\\*ECID_" + rtypeName[tmpCnt] + "\\*", "gi"),
										showTxt(gid));
								tmpCnt++
							}
						}
					...
					group_model = group_model.replace(/\*GROUPNUMBER_SCROLL\*/i, group_id);
					...
					if (!CLUSTERIZE_SW && groupScrollHash["total"] > groupScrollHash[group_id]) group_model =
						...
						group_model.replace(/\*RIGHT_ON\*/i, "");
					group_model = group_model.replace(/\*GROUPNUMBER\*/i, group_id);
					...
					group_model = group_model.replace(/\*groupsheader3\*/i, showTxt(groupsHeaderAry[2]));
		...
		tmpModel = tmpModel.replace(/\*OBT_SHOW\*/i, filterRtypeAry.indexOf(_top.choice_rtype) == -1 && hasEC == "Y" &&
			util.in_array(nowModel, showOBT) && nowModel != "PK" && ec_OBTcount > 2 && isFantasy != "Y" ?
			tmpOBTModel : "");
		tmpModel = tmpModel.replace(/\*DIS_PEN\*/i,
			nowModel == "PK" ? "" : "style='display:none;'");
		tmpModel = tmpModel.replace(/\*BLOCKSCROLL\*/i, !isIOS ? "update" : "");
		tmpModel = tmpModel.replace(/\*SCROLL_LOCK\*/i, "");
		tmpModel = tmpModel.replace(/\*PAGENO\*/i, pageIndex);
		tmpModel = tmpModel.replace(/\*GAMEINDEX\*/i, x);
...
function getPDModel(obj) {
	...
	if (nowPDMode == "choice") {
		...
		tmpModel = tmpModel.replace(new RegExp("\\*SCORE_H\\*", "gi"), hostScore);
		tmpModel = tmpModel.replace(new RegExp("\\*SCORE_C\\*",
			"gi"), customerScore)
	} else {
