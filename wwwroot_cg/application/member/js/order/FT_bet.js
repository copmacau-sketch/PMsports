function FT_bet(_win, _dom) {
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    var classname = "FT_bet.js";
    var timerHash;
    var config_set;
    var _mc = new Object;
    ...
    _self.bet = function(datas) {
        ...
        urlParams = "p=FT_bet&" + top.param + "&" + urlParams;
    ...
    _self.betFinish = function(xml) {
        ...
        try {
            if (xmdObj["code"].innerHTML == "560") {
                ...
                _mc["bet_finish_score"].style.display = "";
                _mc["bet_finish_score"].innerHTML = util_game.needToShowScore(server_wtype) ? score : "";
                ...
                if (util_game.checkWtypeIsR(server_wtype)) {
                    ...
                    _mc["bet_finish_chose_con"].className = color
                } else {
                    _mc["bet_finish_con"].innerHTML = conObj["bet_finish_con"];
                    _mc["bet_finish_con_c"].innerHTML = conObj["bet_finish_con_c"]
                }
             try {
					_mc["orderMsg_div"].style.display = "";

					dg = xmlnode.Node(xmlnode.Root[0], "score_h").innerHTML;
					if (dg > " ") {				
						_mc["bet_finish_dg"].className = "status_word word_yellow";
						_mc["orderMsg_div"].className = "ord_msg ord_levelmsg bg_yellow";
						var msgliPending = document.createElement("li");
						msgliPending.innerHTML = LS_code.get("bet_pending");
						_mc["orderMsg"].appendChild(msgliPending);
						var ticket_id = xmlnode.Node(xmlnode.Root[0], "ticket_id").innerHTML;
						dg_tid = ticket_id;
						_self.checkDanger();
						_self.createDangerTimer(ticket_id);
					setTimeout(function() {
							var pendingMessages = _mc["orderMsg"].getElementsByTagName("li");
							for (var i = 0; i < pendingMessages.length; i++) {
								if (pendingMessages[i].innerHTML === LS_code.get("bet_pending")) {
									_mc["orderMsg"].removeChild(pendingMessages[i]);
									break;
								}
							}
							var msgliSuccess = document.createElement("li");
							msgliSuccess.innerHTML = LS_code.get("bet_success");
							_mc["orderMsg"].appendChild(msgliSuccess);
                ...
                if (!util_game.checkWtypeIsOU(server_wtype) && server_wtype != "W3")
                    ...
                    _mc["bet_finish_chose_con"].innerHTML = _abs + util.showTxt(xmlnode.Node(xmlnode.Root[0], "spread").innerHTML);
                    _mc["bet_finish_chose_con"].style.display = ""
                }
                ...
                _mc["bet_finish_ior"].innerHTML = util.showTxt(tmp_ior);
            ...
            _mc["div_icon_info_bet"] = document.getElementById("div_icon_info_bet");
            if (_self.paramHash["isFantasy"] == "Y") {
                _mc["div_icon_info_bet_i"] = document.getElementById("div_icon_info_bet_i");
                _mc["div_icon_info_bet"].style.display = "";
                util.addEvent(_mc["div_icon_info_bet_i"], "click", _self.AlertFantasyInfo, _self.paramHash["fantasyObj"])
            } else
    ...
    _self.clearDangerTimer = function(xml_key) {
        echo("[clearDangerTimer] = ", classname, " key = ", xml_key);
        if (xml_key == "all") {
    ...
    _self.checkDangerFinish = function(xml) {
        ...
        if (ticket[dg_tid] != null) {
            var _status = util.showTxt(ticket[dg_tid].innerHTML);
            _self.chgDangerStatus(xml_key, _status)
        }
    }
    ;
    _self.chgDangerStatus = function(xml_key, _status) {
        echo("[openBets] = ", top["openBets"], " status = ", _status);
        if (top["openBets"]) {
            var str_sty = "";
            var str_code = "";
            switch (_status) {
                ...
                case "R":
                    ...
                    if (top["dgStatus_hash"][xml_key] == "isToast") {
                        ...
                        return
                    } else if (top["dgStatus_hash"][xml_key] == "isReceipt" && top["isOrderView"]) {
                        parentClass.dispatchEvent("showAlertMsg", {
                            "target": "message_pop_nobtn",
                            "msg": LS_code.get("1more_rejected_toast"),
                            "confirm": "N",
            ...
            _mc["bet_finish_dg"].className = str_sty;
