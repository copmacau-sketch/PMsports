function Util(_win, _dom) {
                ...
                _self.chgCreditAnimation = function(obj) {
                    ...
                    if (count == 1)
                        count = 0;
                    if (parseFloat(top["userData"].maxcredit) >= 500 || oldcash >= 500)
                        if (oldcash > parseFloat(top["userData"].maxcredit) && oldcash - parseFloat(top["userData"].maxcredit) < 500)
                            total = oldcash - parseFloat(top["userData"].maxcredit);
                        else
                            total = 500;
                    else if (oldcash > parseFloat(top["userData"].maxcredit))
                        total = oldcash - parseFloat(top["userData"].maxcredit);
                    else if (oldcash < parseFloat(top["userData"].maxcredit))
                        total = parseFloat(top["userData"].maxcredit) - oldcash;
                    targetCurrency.innerHTML = _self.showTxt(top["userData"].currency);
                    targetCredit.innerHTML = _self.showTxt(Ary[count]["s"]);
                    var tmp = targetCredit.innerHTML * 1;
                    ...
                    var i = setInterval(function() {
                        if (!top.showCredit) {
                            targetCredit.innerHTML = _self.showTxt(LS_code.get("right_msg"));
                            clearInterval(i);
                        ...
                        targetCredit.innerHTML = _self.showTxt(_self.trans_thousand(final));
                        cnt++
                    }, 1);
                    count++;
                    top["orderinfo"]["date"] = "";
                ...
                _self.setDefFilter = function(filterHash) {
                    ...
                    console.log("\u8a2d\u5b9a\u521d\u59cb\u904e\u6ffe\u5bb9\u5668", filterHash);
                    for (var f = 0; f < filterTab.length; f++)
                        if (filterHash && filterHash[filterTab[f]] * 1 > 0) {
                            ...
                            console.log(filterTab[f], "\u6578\u91cf\u70ba0,\u7e7c\u7e8c\u7e5e\u4e0b\u4e00\u500b\u904e\u6ffe\u9801\u7c64");
                    return "noTab"
                }
                ;
                ...
                _self.getKeyValue = function(xmlnode, tmpGame, key, wtypeStr, auto) {
                    ...
                    if (xmlnode != null)
                        ...
                        retVal = tmpGame[wtypeStr][key] ? tmpGame[wtypeStr][key] : "";
                    else
