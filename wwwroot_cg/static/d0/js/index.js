function index(_win, _dom){
    ...
    var cache_html_sw = true; // 開啟首屏 HTML 緩存，減少重複請求
    var topFrame = null;
    var leftFrame = null;
    ...
    var critical_done = false; // 是否已因關鍵 3 幀完成而進入首頁（避免重複 bodyGoToPage）
    ...
    */
    ...
    // 關鍵 3 幀（頂/左/底選單）載完即進首頁，不等待 alert/anno，加快首屏
    _self.check_critical_div = function () {
        if (critical_done) return;
        if (!arr_check_all_div["top_menu_show"] || !arr_check_all_div["left_menu_show"] || !arr_check_all_div["bottom_menu_show"]) return;
        critical_done = true;
        _self.loadAnnounce_Prombel_luminous();
        _self.bodyGoToPage({ "page": "dashboard_main", "retFun": _self.showImportantAnno });
    };

    // 更新各區塊載入狀態（alert/anno 載完僅更新狀態，不再觸發進首頁）
    ...
    _self.chooseLayer=function(){
        ...
        _self.goToPage("alert_choose_show", "alert_choose", function(){
            try{
                ...
                var _co = {"id":"layer_d0","style":"","content":LS.get("acc_co"),"postHash":_postHashCO};
    ...
    _self.clearTimer = function () {
        ...
        if(timerHash["Ann_ProAcc"]!=null) timerHash["Ann_ProAcc"].clearObj();
    }
