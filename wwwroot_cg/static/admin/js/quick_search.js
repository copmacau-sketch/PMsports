function quick_search(_win, _dom ,param) {
    ...
    _self.init = function () {
        ...
        //判斷目前登入的層級 顯示隱藏
        _self.showAndHidden(_mc["search_d0"], "on");
        _self.showAndHidden(_mc["search_co"], "");
    ...
    _self.getViewAllEle = function () {
        ...
        _mc["search_d0"] = dom.getElementById("search_d0");
    ...
    _self.parseData = function (data) {
        ...
        var parseDate = null;
        if (nowSearchType == "d0") {
            parseDate = data["d0_id_data"];
        } else if (nowSearchType == "co") {
        ...
        if (parseDate != null && (parseDate.length > 0) ) {//有資料才跑
            for (var i = 0; i < parseDate.length; i++) {
                ...
                if(tmpdata["usertype"] == "ads"){
                    xmp_contant = xmp_contant.replace("*USERTYPE*", "超管员");
                }else if(tmpdata["usertype"] == "ad"){
                    xmp_contant = xmp_contant.replace("*USERTYPE*", "公司");
                }else if(tmpdata["usertype"] == "d0"){
                    xmp_contant = xmp_contant.replace("*USERTYPE*", "分公司");
                }else{
                ...
                try {
                    switch (nowSearchType){
                        case "mem":
                            ...
                            var upid = data["co_id_data"][upid]["d0_co_id"];
                            if (upid) upD0 = data["d0_id_data"][upid]["username"];//分公司

                            var upid = data["d0_id_data"][upid]["ad_d0_id"];
                            if (upid) upAD = data["ad_id_data"][upid]["username"];//公司
                            break;
                        case "ag":
                            xmp_contant = xmp_contant.replace("*USERTYPE*", LS.get("layer_type_name_" + tmpdata["usertype"]));
                        ...
                        case "su":
                            ...
                            var upid = data["co_id_data"][tmpdata["co_id"]]["d0_co_id"];
                            if (upid) upD0 = data["d0_id_data"][upid]["username"];//分公司
                        ...
                        case "co":
                            ...
                            upD0 = data["d0_id_data"][tmpdata["d0_id"]]["username"];//分公司

                            var upid = data["d0_id_data"][tmpdata["d0_id"]]["ad_d0_id"];
                            if (upid) upAD = data["ad_id_data"][upid]["username"];//公司
                            break;
                        case "d0":
                            upAD = data["ad_id_data"][tmpdata["ad_id"]]["username"];//公司
                            break;
                    }
                } catch (e) {
                ...
                xmp_contant = xmp_contant.replace("*UPSUPERD0*", upD0);//
                xmp_contant = xmp_contant.replace("*UPSUPERAD*", upAD);//
                outdata += xmp_contant;
            }
        }else{
            //如果沒有資料要顯示另一個區塊
    ...
    _self.addStaticListen = function () {
        ...
        util.addEvent(_mc["search_d0"], "click", _self.chgSearchType, "d0");
