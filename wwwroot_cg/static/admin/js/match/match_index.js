function match_index(_win, _dom, _toppar) {
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    var classname = "match_index";
    var util;
    var LS;
    var LS_code;
    var bodyFrame = null;                                                              // match_header
    ...
    selObj["bet"] = new Array("ALL", "Y","N");
    selObj["score"] = new Array("ALL", "Y","N");
    var choBet = "Y";
    var choScore = "N";
    ...
    _self.init = function () {
        util.echo("match_index complete");
        parentClass.dispatchEvent("chgPageName", { "pageType": "match", "uniqText": _self.transUniqText(top.tbet_showtype) });
        if (top.tbet_gtype == undefined) top.tbet_gtype = "FT";
        /*if(top.bet == undefined) top.bet = "Y";
        if (top.score == undefined) top.score = "N";*/
        ...
        if (toppar["score"] == null) {
            toppar["score"] = top.bet;
        } else {
            top.score = toppar["score"];
        }
        choBet = toppar["bet"];
        choScore = toppar["score"];
        ...
        _set["CompleteFunc"] = {};
        _set["CompleteFunc"]["match_model"] = _self.LoadComplete;
        _set["CompleteFunc"]["match_model_allbet"] = _self.LoadAllbetComplete;
        if(_set["layer"]) _set["layer"] = "match_model";

        _self.initSelectFun();
        _self.setSelectEvent1();
        //set 日期列
        _self.getDateList();
        ...
        _self.addEventListener("showMatchEdit", _self.indexShowMatchEdit);
    ...
    _self.subPage = function(par){
        /*if(par["sub_page"] == "match_model_allbet"){
    ...
    _self.indexShowMatchEdit = function (parObj) {
        parentClass.dispatchEvent("showMatchEdit", parObj);
    }
    // ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    ...
    _self.transShowtypeName = function (name) {
        ...
        nameHash[name] = "";
        if (nowTab != "outright") {
            nameHash["MATCH"] = "match";
            nameHash["SCORE"] = "score";
        } else {
            nameHash["MATCH"] = "match_outright";
            nameHash["SCORE"] = "score_outright";
        }

        if (nameHash[name] == null) nameHash[name] = "match";
    ...
    //     _self.scroll_ver_event(dom.getElementById("body_show"),dom.getElementById("match_show"));
    // }
    ...
    _self.getModel = function (date, isInit) {
        ...
        if (isInit) {
            ...
            param["page"] = "match_model";
            param["target"] = "match_model";
            param["retFun"] = _self.getModelComplete;
            param["retParam"] = date;

            param["post"] = "tbet_showtype=" + _self.transShowtypeName("MATCH") + "&tbet_gtype=" + toppar["tbet_gtype"];
            ...
            par["sub_page"] = "match_model";
            par["tab"] = nowTab;
            par["date"] = date;
            param["page"] = "match_header";
    ...
    _self.LoadComplete = function (json) {
        ...
        if (nowTab != "outright") {
            ...
            if (hash["results_data"].length == 0) {
                dom.getElementById("match_show").style.display = "none";
                dom.getElementById("match_nodata").style.display = "";
            } else {
                dom.getElementById("match_show").style.display = "";
                dom.getElementById("match_nodata").style.display = "none";
    ...
    _self.parseData = function (hash) {
        ...
        var objid = ",match_show,sort_model,league_model,";
        var tmpObj = util.getObjAry(dom, objid);
        //console.log(tmpObj);
        _mc = util.mergeArray(_mc, tmpObj);
        if (lazy_page == 1) {
            _mc["match_show"].innerHTML = "";
            _mc["match_show"].innerHTML = _mc["sort_model"].innerHTML;
        ...
        if (nowTab != "outright") {
            for (var i = ((lazy_page - 1) * lazy_count) ; i < (lazy_page*lazy_count) ; i++) {
                ...
                for (var j = 0; j < game_data.length; j++) {
                    ...
                    if(top.login_layer != "ads"){
                        tpl.replace(new RegExp("\\\*EDIT\\\*", "gi"), "style='display:none;'");
                    }
                    ...
                    if (top.tbet_gtype == "FT") {
                        ...
                        if ("GMH" in tmpData) {
                            ...
                            tpl.replace(new RegExp("\\\*SCOREH_G\\\*", "gi"), '');
                            tpl.replace(new RegExp("\\\*SCOREC_G\\\*", "gi"), '');
                        }
                        ...
                        if ("GMH6" in tmpData) {
                            ...
                            tpl.replace(new RegExp("\\\*SCOREH_S\\\*", "gi"), '');
                            tpl.replace(new RegExp("\\\*SCOREC_S\\\*", "gi"), '');
                        }
                        ...
                        if ("GMH" in tmpData) {
                            ...
                            tpl.replace(new RegExp("\\\*GMH_H\\\*", "gi"), '');
                            tpl.replace(new RegExp("\\\*GMH_C\\\*", "gi"), '');
                        }
                        ...
                        if ("HGMH" in tmpData) {
                            ...
                            tpl.replace(new RegExp("\\\*HGMH_H\\\*", "gi"), '');
                            tpl.replace(new RegExp("\\\*HGMH_C\\\*", "gi"), '');
                        }
                        ...
                        if ("GMH" in tmpData) {
                            ...
                            tpl.replace(new RegExp("\\\*SC_GAME_H\\\*", "gi"), '');
                            tpl.replace(new RegExp("\\\*SC_GAME_A\\\*", "gi"), '');
                        }
                ...
                _mc["match_show"].innerHTML += tmp_league_html;
            ...
            for (var i = 0; i < hash.length; i++) {
                ...
                for (var j = 0; j < game_data.length; j++) {
                    ...
                    detailHash["GTYPE"] = top.tbet_gtype;
                    detailHash["GID"] = tmp_data["gid"];
                    detailHash["WTYPE"] = "match";
                    detailHash["TEAM_H"] = tmp_data["team_h"];
                    detailHash["TEAM_C"] = tmp_data["team_c"];
                    detailHash["LEAGUE"] = tmp_data["league_name"];
                    util.addEvent(dom.getElementById("show_details_" + tmp_data["gid"]), "click", _self.gotoBetDetailEvent, { "postHash": detailHash});
                    if(top.login_layer=="ad"){
                         var ss = dom.getElementById("show_edit_" + tmp_data["gid"]);
                         if(ss){
                             ss.remove();
                         }
                    }else{
                        util.addEvent(dom.getElementById("show_edit_" + tmp_data["gid"]), "click", _self.viewEdit, { "gtype": top.tbet_gtype, "gid": tmp_data["gid"], "game_data": tmp_data, "week": choWeek });
                    }
                 }
            }
    ...
    _self.viewEdit = function (e, obj) {
        ...
        parentClass.dispatchEvent("showMatchEdit", param);
    }
    ...
    _self.initScroll = function () {
        ...
        //util.classFunc(dom.getElementById("match_show"), "tbet_title_fixed", "remove");
        if (top.tbet_showtype == "OUTRIGHT") {
            fixed_div_height = 0;
        } else {
            fixed_div_height = dom.getElementById("match_show").children[0].nextElementSibling.clientHeight;
        }
        util.addEvent(dom.getElementById("body_show"), "scroll", _self.scrollVerEvent, dom.getElementById("match_show"));
        now_fix_league = 0;
        var t = new Object();
        t.target = dom.getElementById("body_show");
        _self.scrollVerEvent(t, dom.getElementById("match_show"));
    }
    ...
    _self.reloadFilterData = function () {
        if (nowTab != "outright") {
            var par = "";
            par += top.param;
            par += "&p=get_match_filter_data";
            par += "&totalBets=match_" + nowTab;
    ...
    _self.initSelectFun=function(){
        selectFun["bet"] = _self.chgSel;
        selectFun["score"] = _self.chgSel;
    }
    ...
    _self.setSelectEvent1=function(){
        for(var rtype in selObj){
            ...
            rNow.innerHTML = dom.getElementById(rtype+"_"+toppar[rtype]).innerHTML;
    ...
    _self.gotoBetDetail = function (param,isInit) {
        ...
        //新增進內層的點擊事件
        parentClass.dispatchEvent("showLoading", {"showLoading":true});
        _set["layer"] = "match_wagers_index";

        if(isInit){
            // var par = new Object();
            // par["page"] = "match_wagers_index";
            ...
            param.postHash["sub_page"] = "match_wagers_index";
            param.postHash["sub_target"] = "match_body";

            param.postHash["tbet_showtype"] = toppar["tbet_showtype"];
            param.postHash["tbet_gtype"] = toppar["tbet_gtype"];
            //param.postHash["sub_retFun"] = "match_wagers_index";
            var par = new Object();
