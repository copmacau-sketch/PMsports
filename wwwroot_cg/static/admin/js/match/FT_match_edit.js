function FT_match_edit(_win, _dom, _post) {
    var classname = "FT_match_edit";
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    ...
    var status = 0;
    var aryType = Array("AGMH", "BGMH", "CGMH", "HGMH", "DGMH", "EGMH", "FGMH", "GMH","AGMC", "BGMC", "CGMC", "HGMC", "DGMC", "EGMC", "FGMC", "GMC");
    var postHash = _post;
    ...
    _self.chgSel=function(e, param){
        ...
        if(status>0 || status=="cancal"){
            ...
            for(var i=0;i<aryType.length;i++){
                dom.getElementById(aryType[i]).value = str;
            }
        }else{
            _self.init();
        }
    ...
    _self.save = function(){
        ...
        param += "&status="+status;
        for(var i=0; i < ary.length; i++){
            var v = dom.getElementById(ary[i]).value;
            param += "&"+ary[i]+"="+v;
        }
    ...
    _self.parseData = function (obj) {
        ...
        for (var i = 0; i < score_type_h.length; i++) {
            ...
            tpl.replace(new RegExp("\\\*TEAM_H_SCORE\\\*", "gi"), _self.setInput(score_type_h[i],gameData[score_type_h[i]]));
            if (gameData[score_type_c[i]] == undefined) {
                tpl.replace(new RegExp("\\\*TEAM_C_SCORE\\\*", "gi"), _self.setInput(score_type_c[i],gameData[score_type_h[i]]));
            } else {
                tpl.replace(new RegExp("\\\*TEAM_C_SCORE\\\*", "gi"), _self.setInput(score_type_c[i],gameData[score_type_c[i]]));
            }
