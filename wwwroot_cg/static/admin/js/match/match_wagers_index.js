function match_wagers_index(_win, _dom, paramObj){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    ...
    _self.init=function(){
        try{
            ...
            if(param.sub_getDataPage == "match_wagers_account" || param.sub_getDataPage == "match_wagers_list_bet"){
                _set["nowPage"] =  param.sub_getDataPage;

                if(param.sub_getDataPage == "match_wagers_account")_set["target"] ="viewAccount";
                if(param.sub_getDataPage == "match_wagers_list_bet")_set["target"] ="viewDetails";

            }
            _self.goTarget({"page":"match_wagers_account"},true);
        }catch(e){
            console.log(e);
        }
    }
    ...
    _self.initClickEvent = function(){
        util.addEvent(_mc["viewAccount"], "click", _self.clickTabEvent, {"target":"viewAccount", "page" : "match_wagers_account"});
        util.addEvent(_mc["viewDetails"], "click", _self.clickTabEvent, {"target":"viewDetails", "page" : "match_wagers_list_bet"});
    }

    _self.parseGameInfo = function(dataHash){
        // console.log(dataHash);
        var tmp = _mc["info_model"].cloneNode(true);
        var tmpAry = new Array("LEAGUE","TEAM_H","TEAM_C");
    ...
    _self.goTarget = function(_par,isinit){
        ...
        if(isinit){
            ...
            par.postHash["sub_page"] = "match_wagers_index";
            par.postHash["sub_getDataPage"] = _par["page"];
            par.postHash["sub_target"] = "match_body";
