function acc_ad_detail(_win, _dom, paramObj){
    var _self = this;
    var level = "ad";
    var win = _win;
    var dom = _dom;
    ...
    var co_max = "";
    var co_min = "";
    var classname = "acc_ad_detail";
    acc_title["title"] = new Array("acc_detail","acc_report","acc_eventlog","acc_onlineMem");
    acc_title["perf"] = new Array("perf_yesterday","perf_week","perf_period");
    acc_title["summary"] = new Array("summary_d0","summary_co","summary_su","summary_ag","summary_mem");
    ...
    _self.init=function(){
        ...
        param = paramObj;
        var obj_ids = ",div_acc_detail,div_acc_report,div_acc_eventlog,div_acc_onlineMem,";
        obj_ids += "acc_detail,acc_report,acc_eventlog,acc_onlineMem,"
        obj_ids += "btn_add_acc,btn_cancel,btn_save,";
        obj_ids += "box_user,input_user,";
        obj_ids += "box_alias,input_alias,msg_alias,";
        obj_ids += "box_url,input_url,msg_url,";
        ...
        obj_ids += "box_date,input_create_date,";
        obj_ids += "box_acc_summary,amount_d0,amount_co,amount_su,amount_ag,amount_mem,last_login,add_date,";
        obj_ids += "box_perf,perf_yesterday,perf_week,perf_period,perf_winloss,perf_profit,perf_turnover,";
        obj_ids += "box_down_acc_summary,summary_d0,summary_co,summary_su,summary_ag,summary_mem,amount_active,amount_veiwonly,amount_inactive,amount_suspended,summary_level,";
        obj_ids += "box_psw,input_psw,first_estimate,eye_psw,complexity,psw_light,strengthTxt,msg_psw,psw_btn_cancel,psw_btn_save,";
        obj_ids += "msg_sports_credit,box_top,box_bottom,msg_max_credit,";
        obj_ids += "credit_active,credit_veiwonly,credit_inactive,credit_suspended,div_conn,";
        ...
        util.addEvent(_mc["input_min_m"], "keydown", _self.keep_credits, _mc["input_min_m"]);
        util.addEvent(_mc["input_min_r"], "keydown", _self.keep_credits, _mc["input_min_r"]);
        util.addEvent(_mc["input_min_re"], "keydown", _self.keep_credits, _mc["input_min_re"]);
        util.addEvent(_mc["input_min_dt"], "keydown", _self.keep_credits, _mc["input_min_dt"]);
        util.addEvent(_mc["input_min_rdt"], "keydown", _self.keep_credits, _mc["input_min_rdt"]);
        util.addEvent(_mc["input_min_fs"], "keydown", _self.keep_credits, _mc["input_min_fs"]);

        util.addEvent(_mc["input_max_m"], "keydown", _self.keep_credits, _mc["input_max_m"]);
        util.addEvent(_mc["input_max_r"], "keydown", _self.keep_credits, _mc["input_max_r"]);
        util.addEvent(_mc["input_max_re"], "keydown", _self.keep_credits, _mc["input_max_re"]);
        util.addEvent(_mc["input_max_dt"], "keydown", _self.keep_credits, _mc["input_max_dt"]);
        util.addEvent(_mc["input_max_rdt"], "keydown", _self.keep_credits, _mc["input_max_rdt"]);
        util.addEvent(_mc["input_max_fs"], "keydown", _self.keep_credits, _mc["input_max_fs"]);

        util.ChkKeyCash(_mc["input_min_m"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_min_r"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_min_re"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_min_dt"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_min_rdt"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_min_fs"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });

        util.ChkKeyCash(_mc["input_max_m"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_max_r"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_max_re"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_max_dt"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_max_rdt"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
        util.ChkKeyCash(_mc["input_max_fs"], { initShow: _self.closeErrMsg, onErr: _self.ChkCreditErr, onSuc: _self.show_credits });
    ...
    _self.ChkCreditErr = function (e) {
        if (e.target == _mc["input_min_m"]) {
            _self.showError("sports_credit", LS.get("str_min_m"));
        }

        if (e.target == _mc["input_min_r"]) {
            _self.showError("sports_credit", LS.get("str_min_r"));
        }

        if (e.target == _mc["input_min_re"]) {
            _self.showError("sports_credit", LS.get("str_min_re"));
        }

        if (e.target == _mc["input_min_dt"]) {
            _self.showError("sports_credit", LS.get("str_min_dt"));
        }

        if (e.target == _mc["input_min_rdt"]) {
            _self.showError("sports_credit", LS.get("str_min_rdt"));
        }

        if (e.target == _mc["input_min_fs"]) {
            _self.showError("sports_credit", LS.get("str_fs"));
        }

        if (e.target == _mc["input_max_m"]) {
            _self.showError("sports_credit", LS.get("str_max_m"));
        }

        if (e.target == _mc["input_max_r"]) {
            _self.showError("sports_credit", LS.get("str_max_r"));
        }

        if (e.target == _mc["input_max_re"]) {
            _self.showError("sports_credit", LS.get("str_max_re"));
        }

        if (e.target == _mc["input_max_dt"]) {
            _self.showError("sports_credit", LS.get("str_max_dt"));
        }

        if (e.target == _mc["input_max_rdt"]) {
            _self.showError("sports_credit", LS.get("str_max_rdt"));
        }

        if (e.target == _mc["input_max_fs"]) {
            _self.showError("sports_credit", LS.get("str_max_fs"));
        }
    ...
    _self.ChgSummary = function(tarObj){
        ...
        _mc["summary_level"].innerHTML = inn;
        summary_layer = level;
        _self.getUserCredit();
    }
    ...
    _self.loadUserData = function(){
        var par = "p=get_" + level + "_list" + _self.targetParam("get_list_ad");
        var getHttp = new HttpRequest();
        getHttp.addEventListener("LoadComplete", _self.showUserData);
        getHttp.loadURL(top.url, "POST", par);
    }

    _self.submitCheck = function(isM){
        _self.closeErrMsg();
        if(acc_par.new_alias == ""){
            _self.showError("alias", LS.get("empty_alias"));
            return false;
        }

        if(acc_par.new_url == ""){
            _self.showError("url", LS.get("empty_alias"));
            return false;
        }

        if(acc_par.new_enddate != 0){
            var RegDate = /^[1-9]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/;
            if(!RegDate.test(acc_par.new_enddate)){
                _self.showError("enddate", LS.get("empty_enddate1"));
                return false;
            }
        }

        if(isM){
            if(acc_par.new_im == ""  || acc_par.new_im*1==0){
                _self.showError("sports_credit", LS.get("str_m"));
                return false;
            }
            if(acc_par.new_ir == ""  || acc_par.new_ir*1==0){
                _self.showError("sports_credit", LS.get("str_r"));
                return false;
            }
            if(acc_par.new_iou == ""  || acc_par.new_iou*1==0){
                _self.showError("sports_credit", LS.get("str_ou"));
                return false;
            }
            if(acc_par.new_ipd == "" || acc_par.new_ipd*1==0){
                _self.showError("sports_credit", LS.get("str_pd"));
                return false;
            }
            if(acc_par.new_it == ""  || acc_par.new_it*1==0){
                _self.showError("sports_credit", LS.get("str_t"));
                return false;
            }
            if(acc_par.new_if == ""  || acc_par.new_if*1==0){
                _self.showError("sports_credit", LS.get("str_f"));
                return false;
            }
            if(acc_par.new_ip == ""  || acc_par.new_ip*1==0){
                _self.showError("sports_credit", LS.get("str_p"));
                return false;
            }
            if(acc_par.new_ifs == "" || acc_par.new_ifs*1==0){
                _self.showError("sports_credit", LS.get("str_fs"));
                return false;
            }
            if(acc_par.new_imax == "" || acc_par.new_imax*1==0){
                _self.showError("max_credit", LS.get("str_max"));
                return false;
            }
        }
    ...
    _self.upd_data = function(e){
        ...
        var new_url = _mc["input_url"].value;
        var new_status = _mc["status_list"].value;
        var new_enddate = 0;
        var isM = isEdit = false;
        if(!_mc["input_no_date"].checked){
            new_enddate = _mc["input_enddate"].value;
        }
        if(new_alias != acc_par.alias) isEdit = true;
        if(new_url != acc_par.url) isEdit = true;
        if(new_status != acc_par.status) isEdit = true;
        if(new_enddate != acc_par.enddate) isEdit = true;
        if(acc_par.hasOwnProperty("m_min")){
            isM = true;
        }
        if(isM){
            var new_min_m   = _mc["input_min_m"].value.replace(/\D/g,'');
            var new_min_r   = _mc["input_min_r"].value.replace(/\D/g,'');
            var new_min_re  = _mc["input_min_re"].value.replace(/\D/g,'');
            var new_min_dt  = _mc["input_min_dt"].value.replace(/\D/g,'');
            var new_min_rdt   = _mc["input_min_rdt"].value.replace(/\D/g,'');
            var new_min_fs   = _mc["input_min_fs"].value.replace(/\D/g,'');

            var new_max_m   = _mc["input_max_m"].value.replace(/\D/g,'');
            var new_max_r   = _mc["input_max_r"].value.replace(/\D/g,'');
            var new_max_re  = _mc["input_max_re"].value.replace(/\D/g,'');
            var new_max_dt  = _mc["input_max_dt"].value.replace(/\D/g,'');
            var new_max_rdt   = _mc["input_max_rdt"].value.replace(/\D/g,'');
            var new_max_fs   = _mc["input_max_fs"].value.replace(/\D/g,'');

            if(new_min_m * 1 != acc_par.m_min * 1)     isEdit = true;
            if(new_min_r * 1 != acc_par.r_min * 1)     isEdit = true;
            if(new_min_re * 1 != acc_par.re_min * 1)   isEdit = true;
            if(new_min_dt * 1 != acc_par.dt_min * 1)   isEdit = true;
            if(new_min_rdt * 1 != acc_par.rdt_min * 1)     isEdit = true;
            if(new_min_fs * 1 != acc_par.fs_min * 1)   isEdit = true;

            if(new_max_m * 1 != acc_par.m_max * 1)     isEdit = true;
            if(new_max_r * 1 != acc_par.r_max * 1)     isEdit = true;
            if(new_max_re * 1 != acc_par.re_max * 1)   isEdit = true;
            if(new_max_dt * 1 != acc_par.dt_max * 1)   isEdit = true;
            if(new_max_rdt * 1 != acc_par.rdt_max * 1)     isEdit = true;
            if(new_max_fs * 1 != acc_par.fs_max * 1)   isEdit = true;
        }
        if (isEdit){
            acc_par.new_alias = new_alias;
            acc_par.new_url = new_url;
            acc_par.new_status = new_status;
            acc_par.new_enddate = new_enddate;
            if(isM){
                acc_par.new_min_m = new_min_m;
                acc_par.new_min_r = new_min_r;
                acc_par.new_min_re = new_min_re;
                acc_par.new_min_dt = new_min_dt;
                acc_par.new_min_rdt = new_min_rdt;
                acc_par.new_min_fs = new_min_fs;

                acc_par.new_max_m = new_max_m;
                acc_par.new_max_r = new_max_r;
                acc_par.new_max_re = new_max_re;
                acc_par.new_max_dt = new_max_dt;
                acc_par.new_max_rdt = new_max_rdt;
                acc_par.new_max_fs = new_max_fs;
            }
            var ret = _self.submitCheck(isM);
            if(!ret)	return;
            //util.removeEvent(_mc["btn_save"], "click");

            var par = "p=body_admin_edit" + _self.targetParam("body_admin_edit");
            var getHttp = new HttpRequest();
    ...
    _self.upd_msg = function (data) {
        ...
        if (arr_data.status == "success") {
            parentClass.dispatchEvent("showLoading", { "showLoading": true });
            parentClass.dispatchEvent("showFadeOutMesg",{"text":LS.get("upd_success")});
            _self.loadUserData();
            // _self.closeEvent("save");
        } else if (arr_data.status == "error") {
            if (arr_data.code == "alias") {
                _self.showError("alias", arr_data.msg);
            } else if (arr_data.code == "url") {
                _self.showError("url", arr_data.msg);
            } else if (arr_data.code == "max_credit") {
                _self.showError("max_credit", arr_data.msg);
            } else if (arr_data.code == "sports_credit") {
                _self.showError("sports_credit", arr_data.msg);
            } else {
    ...
    _self.showUserData = function(data){
        ...
        acc_par.url = arr_data["account"]["url"];
        ...
        _mc["input_url"].value = arr_data["account"]["url"];
        ...
        _mc["amount_d0"].innerHTML = arr_data["account"]["amount_d0"];
        _mc["amount_co"].innerHTML = arr_data["account"]["amount_co"];
        _mc["amount_su"].innerHTML = arr_data["account"]["amount_su"];
        _mc["amount_ag"].innerHTML = arr_data["account"]["amount_ag"];
        _mc["amount_mem"].innerHTML = arr_data["account"]["amount_mem"];
        _mc["last_login"].innerHTML = arr_data["account"]["logindate"];

        if(arr_data["account"]["layer"] == "ads"){
            _mc["div_conn"].remove();
            var hh = _mc["box_acc_summary"].innerHTML;
            var nhh = '<div class="box_summ">\n' +
                '                    <span class="title_item"><tt>本期新公司数</tt></span>\n' +
                '                    <tt id="amount_ad" class="text_bold">'+arr_data["account"]["amount_ad"]+'</tt>\n' +
                '                    </td>\n' +
                '                </div>';
            _mc["box_acc_summary"].innerHTML = nhh + hh;
        }else{
            var conn = arr_data["account"]["conn"];
            ...
            acc_par.m_min = conn.m_min;
            acc_par.r_min = conn.r_min;
            acc_par.re_min = conn.re_min;
            acc_par.dt_min = conn.dt_min;
            acc_par.rdt_min = conn.rdt_min;
            acc_par.fs_min = conn.fs_min;

            acc_par.m_max = conn.m_max;
            acc_par.r_max = conn.r_max;
            acc_par.re_max = conn.re_max;
            acc_par.dt_max = conn.dt_max;
            acc_par.rdt_max = conn.rdt_max;
            acc_par.fs_max = conn.fs_max;
        }
    ...
    _self.targetParam = function(targetName){
        ...
        if(targetName == "body_admin_edit"){
            par+="&id="+param.edit_id;
            par+="&username="+acc_par.username;
            par+="&alias="+acc_par.new_alias;
            par+="&url="+acc_par.new_url;
            par+="&enddate="+acc_par.new_enddate;
            if(acc_par.hasOwnProperty("m_min")){

                par += "&min_m=" + acc_par.new_min_m;
                par += "&min_r=" + acc_par.new_min_r;
                par += "&min_re=" + acc_par.new_min_re;
                par += "&min_rdt=" + acc_par.new_min_rdt;
                par += "&min_dt=" + acc_par.new_min_dt;
                par += "&min_fs=" +acc_par.new_min_fs;

                par += "&max_m=" + acc_par.new_max_m;
                par += "&max_r=" + acc_par.new_max_r;
                par += "&max_re=" + acc_par.new_max_re;
                par += "&max_dt=" + acc_par.new_max_dt;
                par += "&max_rdt=" + acc_par.new_max_rdt;
                par += "&max_fs=" + acc_par.new_max_fs;
            }
        ...
        }else if(targetName == "get_list_ad"){
    ...
    _self.getwinlosslimit = function(arr_data){
        var accObj = new Object();
        //if(arr_data["agent"]==null) return true;
        accObj["agent"] = arr_data["agent"];
        accObj["df_agent"] = (typeof arr_data["df_agent"]!="undefined")?arr_data["df_agent"]:null;
        var comax = 100 - config_set.get("CO_WINLOSS_MAX");
        var comin = 100;
        var obj_num = 0;
        if(accObj["df_agent"]!=null){
            if(accObj["agent"].length>=accObj["df_agent"].length){
                obj_num = accObj["agent"].length;
            }else{
                obj_num = accObj["df_agent"].length;
            }
        } else if (accObj["agent"] != null){
            obj_num = accObj["agent"].length;
        }
        ...
        var dfAry = new Array();
        if(accObj["df_agent"]!=null){
            for(var i = 0;i < accObj["df_agent"].length;i++){
                dfAry[ accObj["df_agent"][i]["id"] ] = accObj["df_agent"][i];
            }
        }
        for(var i=0; i<obj_num; i++){

            var winloss_s = accObj["agent"][i]["winloss_s"];
            var winloss_a = accObj["agent"][i]["winloss_a"];

            var df_obj = null;
            if (dfAry[accObj["agent"][i]["id"]] != null) df_obj = dfAry[accObj["agent"][i]["id"]];

            var dfwinloss_s = null;
            var dfwinloss_a = null;
            if(df_obj != null){
                dfwinloss_s = df_obj["dfwinloss_s"];
                dfwinloss_a = df_obj["dfwinloss_a"];
            }

            if(dfwinloss_s!=null) winloss_s = dfwinloss_s;
            if(dfwinloss_a!=null) winloss_a = dfwinloss_a;

            var tmp_max=220-(winloss_s*1+winloss_a*1);
            var tmp_min=250-(winloss_s*1+winloss_a*1);

            if(tmp_max>comax) comax=tmp_max;
            if(tmp_min<comin) comin=tmp_min;
        }

        co_max = 100-comax;
        co_min = (comin>100)?0:100-comin;
        return true;
    }

    _self.initOption = function(targetObj, now, _max, range,_min){
        if(targetObj==null) return;
        if(now !=null && now !="-") now = (100 - now*1);
    ...
    _self.closeErrMsg = function(){
        _mc["msg_sports_credit"].style.display = "none";
        if( _mc["box_top"].classList.contains("accadd_error") ) _mc["box_top"].classList.remove("accadd_error");

        _mc["msg_max_credit"].style.display = "none";
        if (_mc["box_bottom"].classList.contains("accadd_error")) _mc["box_bottom"].classList.remove("accadd_error");
    }

    _self.showError = function(types,msg){
        var ts = ["m","r","ou","pd","f","t","p","fs"];
        if (types=="psw") {
            ...
            _mc["box_"+types].classList.add("accadd_error");
        } else if (types == "max_credit"){
            _mc["box_bottom"].classList.add("accadd_error");
        } else if(types == "sports_credit"){
            _mc["box_top"].classList.add("accadd_error");
        }
        _mc["msg_"+types].innerHTML = msg;
        _mc["msg_"+types].style.display = "";
        if (types == "psw") _mc["input_" + types].focus();
    }
    ...
    _self.isEdit = function () {
        ...
        var new_status = _mc["status_list"].value;
        var new_url = _mc["input_url"].value;
        if ((new_alias != acc_par.alias) || (new_status != acc_par.status)  || (new_url != acc_par.url)) {
            return true;
        } else {
            return false;
        }
