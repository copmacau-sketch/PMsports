function calendar_ag(_win, _dom){
	var _self=this;
	_self.classname = "calendar_ag";
	...
	_self.dateInit=function (dateObj, dateClickObj, parentScreen){
			...
			_mc[_name].addEventListener("DATE_CHOOSE",function(evt,obj){
					...
					var sDate = _self.dateReplace(document.getElementById(input_id+"_start").value,"get");
					var startDate = new Date(sDate);
					var eDate = _self.dateReplace(document.getElementById(input_id+"_end").value,"get");
					var endDate = new Date(eDate);
					var _abs = new Object();
					var isEnd = false;
					var date_tmp = "";
					_mc[_name].close();
					if(choseDate < startDate && _name.indexOf("end")!=-1){
						var target = document.getElementById(photo_id+"_end");
						document.getElementById(input_id+"_start").value = _self.dateReplace(obj.date,"set");
						_abs["x"] = target.offsetLeft+target.clientWidth;
						_abs["y"] = target.offsetTop+target.clientHeight+3;
						date_tmp = _self.dateReplace(document.getElementById(input_id+"_start").value,"get");
						isEnd = true;
						_mc[_name].open(_abs["x"],_abs["y"],eDate,date_tmp,null,isEnd);
					}else if(choseDate > endDate && _name.indexOf("start")!=-1){
						var target = document.getElementById(photo_id+"_end");
						document.getElementById(input_id+"_start").value = _self.dateReplace(obj.date,"set");
						document.getElementById(input_id+"_end").value = _self.dateReplace(obj.date,"set");
						target.click();
					}else{
