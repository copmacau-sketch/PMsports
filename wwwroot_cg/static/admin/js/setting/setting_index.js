function setting_index(_win, _dom, _toppar){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    var classname = "setting_index";
    var util;
    var LS;
    var config_set;
    var Timer;
    ...
    _self.init=function(){
        ...
        util.addEvent(dom.getElementById("btn_set_url"), "click", _self.setUrlEvent);
        util.addEvent(dom.getElementById("btn_set_url_small"), "click", _self.setUrlSmallEvent);

        parentClass.dispatchEvent("showLoading", {"showLoading":false});

    }

    _self.setUrlSmallEvent = function (){
        var input_set_url = dom.getElementById("input_set_url_small").value;
        if(input_set_url!=''){
            var detail_data = dom.getElementById("764_down_wager_show");
            var urls = detail_data.querySelectorAll('input[id^="url_"]');
            for(var i=0;i<urls.length;i++){
                urls[i].value = input_set_url;
            }
        }
    }

    _self.setUrlEvent = function (){
        var input_set_url = dom.getElementById("input_set_url").value;
        if(input_set_url!=''){
            var detail_data = dom.getElementById("764_up_wager_show");
            var urls = detail_data.querySelectorAll('input[id^="url_"]');
            for(var i=0;i<urls.length;i++){
                urls[i].value = input_set_url;
            }
    ...
    _self.saveEvent = function (mouseEvent,param){
        ...
        for(var i=0;i<k;i++){
            ...
            var user = tr.querySelectorAll('input[id^="user_"]')[0].value;
            var pwd = tr.querySelectorAll('input[id^="pwd_"]')[0].value;
            if(user!="" && pwd != ""){
                paramHash[i] = {
                    user:user,
                    pwd:pwd,
                    id:tr.querySelectorAll('input[id^="mid_"]')[0].value,
                    uid:tr.querySelectorAll('input[id^="uid_"]')[0].value,
                    url:tr.querySelectorAll('input[id^="url_"]')[0].value,
                    status:tr.querySelectorAll('input[id^="status_"]')[0].value,
                    ver:tr.querySelectorAll('input[id^="ver_"]')[0].value,
                };
                isOK = "Y";
            }
        }
    ...
    _self.newSmallEvent = function () {
        ...
        str_small = str_small.replace(new RegExp("\\\*UID\\\*", "gi"), "ssss");
        str_small = str_small.replace(new RegExp("\\\*USER\\\*", "gi"), "");
        str_small = str_small.replace(new RegExp("\\\*MID\\\*", "gi"), "");
        str_small = str_small.replace(new RegExp("\\\*PWD\\\*", "gi"), "");
        str_small = str_small.replace(new RegExp("\\\*URL\\\*", "gi"), "");
        str_small = str_small.replace(new RegExp("\\\*STATUS\\\*", "gi"), "ok");
        str_small = str_small.replace(new RegExp("\\\*VER\\\*", "gi"), "");
    ...
    _self.newEvent = function () {
        ...
        str = str.replace(new RegExp("\\\*UID\\\*", "gi"), "ssss");
        str = str.replace(new RegExp("\\\*USER\\\*", "gi"), "");
        str = str.replace(new RegExp("\\\*MID\\\*", "gi"), "");
        str = str.replace(new RegExp("\\\*PWD\\\*", "gi"), "");
        str = str.replace(new RegExp("\\\*URL\\\*", "gi"), "");
        str = str.replace(new RegExp("\\\*STATUS\\\*", "gi"), "ok");
        str = str.replace(new RegExp("\\\*VER\\\*", "gi"), "");
    ...
    _self.btnDeleteClick = function (){
        ...
        for(var k in dis){
            if(typeof(dis[k])=="object"){
                util.addEvent(dis[k], "click", _self.delEvent, {id:dis[k].getAttribute("id"),type:"big"});
            }
        }
    ...
    _self.delEvent = function (mouseEvent,param){
        ...
        if(did>0 && toppar["setting_showtype"]=="AG"){
    ...
    _self.loadFinish=function(json){
        ...
        for(var i in hash){
            ...
            var str_small = small;
            str = str.replace(new RegExp("\\\*ID\\\*", "gi"), i);
            str = str.replace(new RegExp("\\\*UID\\\*", "gi"), d["uid"]);
            str = str.replace(new RegExp("\\\*USER\\\*", "gi"), d["user"]);
            str = str.replace(new RegExp("\\\*MID\\\*", "gi"), d["mid"]);
            str = str.replace(new RegExp("\\\*PWD\\\*", "gi"), d["pwd"]);
            str = str.replace(new RegExp("\\\*URL\\\*", "gi"), d["url"]);
            str = str.replace(new RegExp("\\\*STATUS\\\*", "gi"), d["status"]);
            str = str.replace(new RegExp("\\\*VER\\\*", "gi"), d["ver"]);


            str_small = str_small.replace(new RegExp("\\\*ID\\\*", "gi"), i);
            str_small = str_small.replace(new RegExp("\\\*UID\\\*", "gi"), d["uid"]);
            str_small = str_small.replace(new RegExp("\\\*USER\\\*", "gi"), d["user"]);
            str_small = str_small.replace(new RegExp("\\\*MID\\\*", "gi"), d["mid"]);
            str_small = str_small.replace(new RegExp("\\\*PWD\\\*", "gi"), d["pwd"]);
            str_small = str_small.replace(new RegExp("\\\*URL\\\*", "gi"), d["url"]);
            str_small = str_small.replace(new RegExp("\\\*STATUS\\\*", "gi"), d["status"]);
            str_small = str_small.replace(new RegExp("\\\*VER\\\*", "gi"), d["ver"]);
            if(d["id"]){
