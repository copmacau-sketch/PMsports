function acc_mem_comm(_win, _dom, param) {
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    ...
    _self.init = function () {
        userDate = param;
        util.echo("acc_mem_comm load complete");
        ...
        var obj_ids = ",commission_info,gtype_menu,FT_commis_info,BK_commis_info,OP_commis_info,FS_commis_info";
            obj_ids += ",FT_R_war_error,FT_RE_war_error,FT_R_bet_error,FT_RE_bet_error,FT_M_bet_error,FT_DT_bet_error,FT_RDT_bet_error";
            obj_ids += ",BK_R_war_error,BK_RE_war_error,BK_R_bet_error,BK_RE_bet_error,BK_M_bet_error,BK_DT_bet_error";
            obj_ids += ",OP_R_war_error,OP_RE_war_error,OP_R_bet_error,OP_RE_bet_error,OP_M_bet_error,OP_DT_bet_error";
            obj_ids += ",FS_FS_bet_error,";
        _mc = util.getObjAry(dom, obj_ids);
        //各球類 限額 退水
        //足球
        var obj_ids = ",FT_R_WARTYPE,FT_R_WAR,FT_R_SC,FT_R_SC_limit,FT_R_SO,FT_R_SO_limit";
        obj_ids += ",FT_RE_WARTYPE,FT_RE_WAR,FT_RE_SC,FT_RE_SC_limit,FT_RE_SO,FT_RE_SO_limit";
        ...
        //籃球
        obj_ids += ",BK_R_WARTYPE,BK_R_WAR,BK_R_SC,BK_R_SC_limit,BK_R_SO,BK_R_SO_limit";
        obj_ids += ",BK_RE_WARTYPE,BK_RE_WAR,BK_RE_SC,BK_RE_SC_limit,BK_RE_SO,BK_RE_SO_limit";
        obj_ids += ",BK_M_SC,BK_M_SC_limit,BK_M_SO,BK_M_SO_limit";
        obj_ids += ",BK_DT_SC,BK_DT_SC_limit,BK_DT_SO,BK_DT_SO_limit,";

        //其他球類
        obj_ids += ",OP_R_WARTYPE,OP_R_WAR,OP_R_SC,OP_R_SC_limit,OP_R_SO,OP_R_SO_limit";
        obj_ids += ",OP_RE_WARTYPE,OP_RE_WAR,OP_RE_SC,OP_RE_SC_limit,OP_RE_SO,OP_RE_SO_limit";
    ...
    _self.setSubmitListerner = function (evt, param) {
        ...
        for (var i in confObj) {
            if (i.indexOf(param.grType) != -1 && i.indexOf("limit") == -1) {
                if (i.indexOf("WARTYPE") != -1) continue;
                var tmpStr = i.split("_");
                var keyName = tmpStr[2];
                if (keyName == "WAR"){
                   usrChgObj["war_set"] = confObj[i].value;        //退水設定
                }else if(i.indexOf("SC") != -1 || i.indexOf("SO") != -1){
                    usrChgObj[keyName] = confObj[i].value.replace(/[^\d]*/g,"");
                }else{
                    usrChgObj[keyName] = confObj[i].value;          //單場/下注 額設定
                }
            }
        }
    ...
    _self.amendCommToServer = function () {
        ...
        urlParams += "&pay_type=" + userDate["pay_type"];
        urlParams += "&act=Y";
        urlParams += "&langx=" + top.langx;
        urlParams += _self.getUsrChgParam();
        urlParams = "p=get_acc_mem_comm&ver=" + top.ver + "&" + urlParams;
    ...
    _self.setViewParam = function (data) {
        ...
        var currency_def = arr_data.currency_def;
        var currency_name = arr_data.currency_name;
        usrOriObj = arr_data["dataAry"];
        if (usrOriObj) {
            for (var i in usrOriObj) {
                var namsSplit = i.split("_");
                if (!confObj[i]) continue;

                if (namsSplit[3] == "limit") { //限額字體
                    var tmp = Math.floor(usrOriObj[i] * 1 / currency_def * 1);
                    //console.log(usrOriObj[i]+"=="+currency_def);
                    /*if (tmp > 10) {
                        tmp = Math.floor(tmp / 10) * 10;
                    }*/
                    confObj[i].innerHTML = LS.get("max_limit_head") + currency_name+ " " + util.mprintf(tmp, 0, 2, false, true);
                    continue;
                }
                ...
                if (confObj[i].tagName == "TT") {
                    confObj[i].innerHTML = usrOriObj[i];
                    continue;
                }
            }
            if (useInitView) _self.ShowAndHidden(_mc["FT_commis_title"], _mc["FT_commis_title"]);   //開啟預設畫面
        }
        //關閉所有編輯狀態
        // for (var i in confObj) {
        ...
        // }
        //關閉畫面的按鈕與提示匡
        // for (var j in classchg) {
        //     classchg[j].classList.remove("edit_on");
        // }
        //關閉當前save視窗
        if (_editComm != null) {
    ...
    _self.vanishStyle_Text = function (grString) {
        if (_mc[grString + "WAR"]) _mc[grString + "WAR"].parentElement.parentElement.classList.remove("accadd_error");
