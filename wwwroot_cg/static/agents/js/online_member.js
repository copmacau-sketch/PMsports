function online_member(_win, _dom){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var parentClass;
    ...
    _self.init=function(){
        ...
        parentClass.dispatchEvent("chgPageName", { "pageName": "onlinemem" });
    ...
    _self.connectComplete=function(data){
        ...
        try{
            ...
            if(arr_data["msg"] == "nodata"){
                ...
                //dom.getElementById("div_show_nodata").innerHTML = dom.getElementById("div_show_nodata").innerHTML.replace(/\*SREACH\*/g, view_sreach_td);
                ...
                //dom.getElementById("div_show").innerHTML = dom.getElementById("div_show").innerHTML.replace(/\*SREACH\*/g, view_sreach_td);
    ...
    _self.parseData = function (hash) {
        ...
        var memAry = new Array("memcount","username","loginname","alias","lastdate","id","onlines");
        for(var i=0; i<hash.length; i++){
            tpl.addBlock("onlineM");
            for(var a=0; a<memAry.length; a++){
                var keys = memAry[a];
                if(keys == "onlines") {
                    if (hash[i].hasOwnProperty("onlines")) {
                        var md = "<tt id=\"btn_out\" class=\"word_blue hand\">提线</tt>";
                        var md_small = "<tt id=\"btn_small_out\" class=\"word_blue hand\">提线</tt>";
                        var f = "Y";

                        for (var b in hash[i]["onlines"]) {
                            if (b == "domain") {
                                if (hash[i]["ip"]) {
                                    if (hash[i]["ip"] != "" && hash[i]["ip"] != "-") {
                                        var str = '<a href="http://www.ip38.com/ip.php?ip=' + hash[i]["ip"] + '" target="_blank"  class="word_blue" title="点击查看IP归属地">' + hash[i]["ip"] + '</a>';
                                        var str_small = str;
                                        if (hash[i]["onlines"]["log"] && hash[i].hasOwnProperty("sp")) {
                                            str += "/";
                                            str += '<span class="word_blue" title="点击查看活动轨迹">';

                                            var kk = '<tt id="btn_sp" class="word_blue hand">不相同</tt>';
                                            if (hash[i]["sp"] == 1) {
                                                kk = '<tt id="btn_sp" class="word_red hand">相同</tt>';
                                            }
                                            str += kk + '</span>';

                                            str_small += "/";
                                            str_small += '<span class="word_blue" title="点击查看活动轨迹">';

                                            var kk = '<tt id="btn_sp_small" class="word_blue hand">不相同</tt>';
                                            if (hash[i]["sp"] == 1) {
                                                kk = '<tt id="btn_sp_small" class="word_red hand">相同</tt>';
                                            }
                                            str_small += kk + '</span>';
                                        }
                                        tpl.replace(new RegExp("\\\*IP\\\*", "gi"), str);
                                        tpl.replace(new RegExp("\\\*IPSMALL\\\*", "gi"), str_small);
                                    } else {
                                        tpl.replace(new RegExp("\\\*IP\\\*", "gi"), "");
                                        tpl.replace(new RegExp("\\\*IPSMALL\\\*", "gi"), "");
                                    }
                                }else{
                                    tpl.replace(new RegExp("\\\*IP\\\*", "gi"), "");
                                    tpl.replace(new RegExp("\\\*IPSMALL\\\*", "gi"), "");
                                ...
                                if (hash[i]["url"]) {
                                    tpl.replace(new RegExp("\\\*URL\\\*", "gi"), hash[i]["url"]);
                                } else {
                                    tpl.replace(new RegExp("\\\*URL\\\*", "gi"), "");
                                }


                            } else {
                                var ma = hash[i]["onlines"][b];
                                if (f == "Y") {
                                    f = "N";
                                }
                                if(b!="log") {
                                    md += " / <tt id=\"btn_" + b + "\" class=\"word_blue hand\">" + ma + "</tt>";
                                    md_small += "<br><tt id=\"btn_small_" + b + "\" class=\"word_blue hand\">" + ma + "</tt>";
                                }
                            }
                        }

                        if (f == "N") {
                            tpl.replace(new RegExp("\\\*MANGER_DATA\\\*", "gi"), md);
                            tpl.replace(new RegExp("\\\*MANGER_DATA_SMALL\\\*", "gi"), md_small);
                        }
                    }else{ 
                        tpl.replace(new RegExp("\\\*IP\\\*", "gi"), "");
                        tpl.replace(new RegExp("\\\*IPSMALL\\\*", "gi"), "");
                        tpl.replace(new RegExp("\\\*URL\\\*", "gi"), "");
                        tpl.replace(new RegExp("\\\*MANGER_DATA\\\*", "gi"), "");
                        tpl.replace(new RegExp("\\\*MANGER_DATA_SMALL\\\*", "gi"), "");
    ...
    _self.initEvent = function (ArrObj, div_show) {
        for (var i = 0; i < ArrObj.length; i++) {
            ...
            var obj_ids = ",btn_out,btn_dobet,btn_mes,btn_sp,btn_chat";
            obj_ids+= ",btn_small_out,btn_small_dobet,btn_small_mes,btn_small_sp,btn_small_chat,";
            var _ary = util.getObjAry(div, obj_ids);
            util.addEvent(_ary["btn_out"], "click", _self.outEvent, obj);
            util.addEvent(_ary["btn_dobet"], "click", _self.betEvent, obj);
            util.addEvent(_ary["btn_mes"], "click", _self.proNewsEvent, obj);
            util.addEvent(_ary["btn_chat"], "click", _self.proChatEvent, obj);
            util.addEvent(_ary["btn_sp"], "click", _self.spOpen, obj);
            util.addEvent(_ary["btn_small_out"], "click", _self.outEvent, obj);
            util.addEvent(_ary["btn_small_dobet"], "click", _self.betEvent, obj);
            util.addEvent(_ary["btn_small_mes"], "click", _self.proNewsEvent, obj);
            util.addEvent(_ary["btn_small_chat"], "click", _self.proChatEvent, obj);
            util.addEvent(_ary["btn_small_sp"], "click", _self.spOpen, obj);
        }
    ...
    _self.betEvent = function (mouseEvent, _param){

        var paramHash = new Object();
        paramHash["username"] = _param["username"];
        paramHash["back_page"] = "online_member";
        var param = new Object();
        param["page"] = "bet_mem_index";
        param["target"] = "bet_body";
        param["postHash"] = paramHash;
        parentClass.dispatchEvent("bodyGoToPage", param);
    }

    _self.proNewsEvent = function (mouseEvent, _param){
        var paramHash = new Object();
        paramHash["username"] = _param["username"];
        paramHash["scoll_type"] = "proNews";
        paramHash["back_page"] = "announcement";
        paramHash["scoll_date"] = "all";
        paramHash["search"] = "";
        paramHash["sort"] = "desc";
        var param = new Object();
        param["page"] = "ann_add";
    ...
    _self.proChatEvent = function (mouseEvent, _param){
        var paramHash = new Object();
        paramHash["username"] = _param["username"];
        paramHash["scoll_type"] = "proChat";
        /*paramHash["back_page"] = "announcement";
        paramHash["scoll_date"] = "all";
        paramHash["search"] = "";
        paramHash["sort"] = "desc";*/
        var param = new Object();
        param["page"] = "announcement";
        param["postHash"] = paramHash;
