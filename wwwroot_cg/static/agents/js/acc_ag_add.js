function acc_ag_add(_win, _dom, _paramObj){
    var _self = this;
    var win = _win;
    var dom = _dom;
    var level = "ag";
    ...
    _self.init=function(){
        ...
        parentClass.dispatchEvent("chgPageName", { "pageType": "account","pageName": "ag_add" });
        ...
        obj_ids += "sel_upper,upper_user,super_agent,";
        obj_ids += "box_acc_type,msg_acc_type,";
        obj_ids += "scroll_div,";
        obj_ids += "possess_tip,";
        obj_ids += "show_account_type,";
        _mc = util.getObjAry(dom, obj_ids);
        if (top.login_layer == "su") _mc["super_agent"].style.display = "none";
        ...
        _mc["acc_type"] = dom.getElementsByName("acc_type") ;
        for (i = 0; i < _mc["acc_type"].length; i++) {
            util.addEvent(_mc["acc_type"][i], "click", _self.chk_cash_sw, { "obj_": _mc["acc_type"],"value": _mc["acc_type"][i].value });
        }
        ...
        ctlObj["acc_type"] = new Object();
        ctlObj["acc_type"].box = _mc["box_acc_type"];
        ctlObj["acc_type"].msg = _mc["msg_acc_type"];
        ...
        _mc["btn_save"].backPage = "acc_ag_list";
        _mc["btn_new"].backPage = "acc_ag_add";
    ...
    _self.closeEvent = function (e) {
        var back_param = new Array();
        back_param["up_id"] = paramObj.up_id;
        var idx = _mc["sel_upper"].selectedIndex;
        back_param["up_username"] = _mc["sel_upper"].options[idx].text;
        var obj = new Object();
        obj.postHash = back_param;
    ...
    _self.loadConfFinish = function (data) {
        ...
        cash_sw = arr_data.user["cash_sw"];
        // 現金開關沒有開 選項要反灰
        for (i = 0; i < _mc["acc_type"].length; i++) {
            if (cash_sw != "Y" && _mc["acc_type"][i].value == 1) {
                _mc["acc_type"][i].parentElement.classList.add("noused");
            }
        }
        _mc["balance"].org_value = balance * 1;
        _mc["balance"].innerHTML = util.mprintf(balance * 1, 0, 2, false, true);

        _mc["cotype"] = util.showTxt(xmlObj["user"].cotype);
    ...
    _self.chgUpperLayerEvent = function (changeEvent, targetObj) {
        ...
        var idx = _mc["sel_upper"].selectedIndex;
        _mc["upper_user"].innerHTML = _mc["sel_upper"].options[idx].text.substring(0, 3);
        _self.loadUpperConf();
    }
    ...
    _self.saveEvent = function (mouseEvent, targetObj) {
        ...
        dom.getElementById("copyArea").innerHTML = LS.get("copy_user")+_mc["upper_user"].innerHTML+ctlObj["user"].input.value+"<br>"+LS.get("copy_pwd")+ctlObj["passwd"].input.value;
    ...
    _self.saveFinish = function (data, targetObj) {
        ...
        if (_status == "error") {
            ...
            if (targetObj.id == "btn_new"){
                ...
                paramObj.back_page = "acc_ag_list";
    ...
    _self.getSaveParam = function () {
        ...
        par += "&pay_type=" + _self.get_acc_type();
        return par;
    }
    ...
    // 判斷現金帳號開關是否有開啟
    _self.chk_cash_sw = function (e, targetObj) {
        _self.clearErrorStatusCtl(null, ctlObj["acc_type"]);

        if (cash_sw!="Y" && targetObj.value==1){
            var obj = new Object();
            obj.msg = LS.get("cash_sw_in_add_user");
            obj.backPage = "account";
            parentClass.dispatchEvent("showAlertMsg", obj);

            var obj = targetObj.obj_ ;
            for (i = 0; i < obj.length; i++) {
                if (obj[i].value==0){
                    obj[i].checked = true ;
                }else{
    ...
    _self.submitCheck = function () {
        ...
        for (var key in ctlObj) {
            if (key == "acc_type"){
                // 額度模式為必填 改為沒有預設, 需檢查是否有選擇
                if (_self.get_acc_type()=="") {
        ...
        if ((ctlObj["credit"].input.value.replace(/\D/g, ''))* 1 == 0 && _self.get_acc_type()==0 ) {
            if(top.langx != "en-us"){
