...
class Account extends Base
{
    ...
    public function chg_psw_ag(){
        ...
        if(empty($this->param["old_pwd"]) || $this->param["old_pwd"] != $this->user["pw"]){
        ...
        try{
            $this->dbc->update(Constant::T_RANK,["pw" => $pwd,"passwd" =>md5Pwd($pwd),"pwddate"=>time()],"`id`={$this->user["id"]}");
            $ary["status"] = "success";
