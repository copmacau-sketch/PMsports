 ...
 *
 * 输出最终经 base64(gzdeflate(xml)) 编码后写入 db_markets.foot_match_xml.r_cn 列。
 * 解码侧由 vendor/common/function.php:de_string() 处理，SPA 的 game_more / 列表
 * 解析器 (worker.js + game_list.js) 期望此完整 XML 格式 — 即与 db_markets/
...
class OddsApiToCrownXml
{
    ...
    public function buildR(array $event, array $odds, string $bookmaker = 'Bet365', string $langx = 'zh-cn', int $internalLid = 0): string
    {
        ...
        foreach ($this->markets as $m) {
            $name = $m['name'] ?? '';
            if ($name === '') continue;
            // 后到的（如 "Bet365 (no latency)") 不覆盖先到的主键
    ...
    private function blockCorrectScore(): string
    {
        ...
        foreach ($rows as $row) {
            ...
            // "1-0", "0-1", "3-3"
            if (!preg_match('/^(\d+)\s*-\s*(\d+)$/', $label, $m)) continue;
            $h = (int)$m[1];
            $c = (int)$m[2];
            $tag = "H{$h}C{$c}";
            $odds[$tag] = $price;
