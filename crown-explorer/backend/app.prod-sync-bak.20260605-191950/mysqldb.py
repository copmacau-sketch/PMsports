...
def derive_status(row: dict) -> str:
    ...
    if started:
        return "inplay"
    if has_live_score and age is not None and 0 <= age <= LIVE_WINDOW_SEC:
        return "inplay"
    if age is not None and 0 <= age <= LIVE_WINDOW_SEC:
        # Kickoff is in the past but within the live window and Crown
        # hasn't flipped status yet — typically a cron-lag situation.
        return "inplay"
    return "pending"
